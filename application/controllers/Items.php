...
class Items extends Secure_area implements Idata_controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model('Inventory_snapshot');
    ...
    function save_category($category_id = FALSE)
    {
        ...
        $enable_mmj_weight = $this->input->post('enable_mmj_weight');
        ...
        if ($category_id = $this->Category->save($category_name, $hide_from_grid, $parent_id, $category_id, $category_color, $category_image_id, 0, $exclude_from_e_commerce, $category_info_popup,$enable_mmj_weight)) {
	...
	*/
    ...
    function view($item_id = -1, $sale_or_receiving = 'sale')
    {
        ...
        $data['supplier_brands']= $data['item_info']->supplier_id ? $this->Supplier_brands->getSupplierBrandsForSelect($data['item_info']->supplier_id) : [];
    ...
    function inventory($item_id = -1, $offset = 0)
    {
        ...
        $cat_id = $this->Item->get_category_id($item_id);
        $cat_info = $this->Category->sold_type_and_mmj_info($cat_id);
        if ($item_id != -1) {
            $data['next_item_id'] = $this->Item->get_next_id($item_id);
            $data['prev_item_id'] = $this->Item->get_prev_id($item_id);
        }
    ...
    function save($item_id = -1)
    {
        ...
        $item_data = array(
            ...
            'brand_id' => $this->input->post('brand_id') == '' ? null : $this->input->post('brand_id'),
            ...
            'product_id' => json_encode($this->input->post('product_id')) == '' ? null : json_encode($this->input->post('product_id')),
            ...
            'mmj_weight' => $this->input->post('mmj_weight') ? $this->input->post('mmj_weight') : null,
        ...
        if ($this->Item->save($item_data, $item_id)) {
            ...
            if ($this->input->post('serial_numbers') && is_array($this->input->post('serial_numbers'))) {
                $this->Item_serial_number->save($item_id, $this->input->post('serial_numbers'), $this->input->post('serial_number_cost_prices'), $this->input->post('serial_number_qty'), $this->input->post('serial_number_date'));
            } else {
    ...
    function transfer_items($item_id)
    {
        $this->load->model('Item_variations');
        $redirect = $this->input->post('redirect');
        $progression_post = $this->input->post('progression');
        ...
        $this->load->model('Item_location');
        $item_value_to_transfer = $this->input->post('transfer_value');
        $item_transfer_mode = $this->input->post('transfer_mode');
        if(($item_transfer_mode !== '') && ($item_value_to_transfer !== ''))
        {                 
            $cur_item_location_info = $this->Item_location->get_info($item_id);
            $trans_safe = 0;  $trans_floor = 0; 
            $current_safe = $cur_item_location_info->safe; 
            $current_floor = $cur_item_location_info->floor; 
            if($item_transfer_mode == 'safe_to_floor'){                   
                $trans_safe = '-'.$item_value_to_transfer;
                $trans_floor = $item_value_to_transfer;
                $current_safe = $current_safe - $item_value_to_transfer;                    
                $current_floor = $current_floor + $item_value_to_transfer;                   
            }else{                   
                $trans_floor = '-'.$item_value_to_transfer;
                $trans_safe = $item_value_to_transfer;
                $current_floor = $current_floor - $item_value_to_transfer;                    
            ...
            (
                ...
                'trans_comment' => ($item_transfer_mode == 'safe_to_floor') ? 'Safe To Floor' : 'Floor To Safe',
                'trans_inventory_safe' => $trans_safe,
                'trans_inventory_floor' => $trans_floor,
                'location_id' => $location_id,
                'trans_current_safe' =>  $current_safe,
                'trans_current_floor' => $current_floor,
                'trans_type' => 'Inventory'
            );
            $this->Inventory->insert($inv_data);

            //Update stock quantity
            if ($this->Item_location->save_quantity($current_safe,$current_floor,$item_id)) {                    
                ...
                if (isset($this->ecom_model)) {
                    ...
                    if ($cur_item_info->is_ecommerce && $location_id == $this->ecom_model->ecommerce_store_location && $total_locations_ecom_sync == 1) {
                        ...
                        $ecom_item_data = array(
                            'stock_safe' => $current_safe,
                            'stock_floor' => $current_floor,
                            'manage_stock' => true
                        );
        ...
        if ($error) {
            echo json_encode(array('success' => false, 'message' => lang('common_error_transfer_item') . ' ' . H($cur_item_info->name), 'item_id' => -1));
        } else {
            echo json_encode(array('success' => true, 'message' => lang('common_item_successful_transferring') . ' ' . H($cur_item_info->name), 'item_id' => $item_id,'redirect' => $redirect, 'progression' => $progression, 'quick_edit' => $quick_edit, 'reload' => true));
        }
    }
    ...
    function save_inventory($item_id = -1)
    {
        ...
        if ( ( $this->input->post("damaged_safe") || $this->input->post("damaged_floor") ) ) {
            $this->Item->save_damaged_qty(date('Y-m-d H:i:s'), $this->input->post("damaged_safe"), $this->input->post("damaged_floor"), $item_id, NULL, $location_id);
        }
        ...
        (
            ...
            'trans_inventory_safe' => $this->input->post('add_subtract_safe'),
            'trans_inventory_floor' => $this->input->post('add_subtract_floor'),
            'location_id' => $location_id,
            'trans_current_safe' => ($cur_item_location_info->safe ? $cur_item_location_info->safe : 0) + ($this->input->post('add_subtract_safe') ? $this->input->post('add_subtract_safe') : 0),
            'trans_current_floor' => ($cur_item_location_info->floor ? $cur_item_location_info->floor : 0) + ($this->input->post('add_subtract_floor') ? $this->input->post('add_subtract_floor') : 0),
            'trans_type' => 'Inventory'
        );
        $this->Inventory->insert($inv_data);

        //Update stock quantity
        if ($this->Item_location->save_quantity(
                ($cur_item_location_info->safe ? $cur_item_location_info->safe : 0) + ($this->input->post('add_subtract_safe') ? $this->input->post('add_subtract_safe') : 0), 
                ($cur_item_location_info->floor ? $cur_item_location_info->floor : 0) + ($this->input->post('add_subtract_floor') ? $this->input->post('add_subtract_floor') : 0), 
                $item_id)) {
            //Ecommerce
            if (isset($this->ecom_model)) {
                $total_locations_ecom_sync = count($this->Appconfig->get_ecommerce_locations());
                ...
                if ($cur_item_info->is_ecommerce && $location_id == $this->ecom_model->ecommerce_store_location && $total_locations_ecom_sync == 1) {
                    $ecom_item_data = array(
                        'stock_safe' => ($cur_item_location_info->safe ? $cur_item_location_info->safe : 0) + ($this->input->post('add_subtract_safe') ? $this->input->post('add_subtract_safe') : 0),
                        'stock_floor' => ($cur_item_location_info->floor ? $cur_item_location_info->floor : 0) + ($this->input->post('add_subtract_floor') ? $this->input->post('add_subtract_floor') : 0),
                        'manage_stock' => true,
                    );
    ...
    function _excel_get_header_row($include_location_columns = 0)
    {
        ...
        $header_row[] = lang('common_source_package_number');
        $header_row[] = lang('common_package_number');
        ...
        if (count($authed_locations) == 1) {
            $header_row[] = lang('items_safe');
            $header_row[] = lang('items_floor');
        }
        ...
        if ($include_location_columns) {
            foreach ($authed_locations as $location_id) {
                ...
                if (count($authed_locations) != 1) {
                    $header_row[] = $location_info->name . ' ' . lang('items_safe');
                    $header_row[] = $location_info->name . ' ' . lang('items_floor');
    ...
    function inventory_print_list($export_excel = 0)
    {
        ...
        if(isset($_POST['clear_filter'])){
            $this->session->unset_userdata('inventory_print_start_date');
            $this->session->unset_userdata('inventory_print_to_date');
        }
        ...
        if(isset($_POST['submit_filter'])){
            ...
            // print_r($_POST);
            // echo "</pre>";
            $sessionData = ['inventory_print_start_date'=>$_POST['inventory_print_start_date'],
                                'inventory_print_to_date'=>$_POST['inventory_print_to_date']]; 
        ...
        $inventory_print_start_date = $this->session->userdata('inventory_print_start_date') ? $this->session->userdata('inventory_print_start_date') : "";
        $inventory_print_to_date = $this->session->userdata('inventory_print_to_date') ? $this->session->userdata('inventory_print_to_date') : "";
        ...
        if ($export_excel) {
            ...
            for ($k = 0; $k < count($data); $k++) {
                ...
                $data[$k]['safe'] = to_quantity($data[$k]['safe']);
                $data[$k]['floor'] = to_quantity($data[$k]['floor']);
            ...
            $header_row = array(
                ...
                'safe' => lang('common_safe'),
                'floor' => lang('common_floor'),
            );
            ...
            $this->load->view("items/inventory_print_list", array('items' => $data,'inventory_print_start_date'=>$inventory_print_start_date,'inventory_print_to_date'=>$inventory_print_to_date));
        }
    }
    ...
    {        
        ...
        foreach ($data as $r) {
            ...
            $p_id = '';
            $product_ids = json_decode($r->product_id);
            ...
            $to_export[lang('common_source_package_number')][] = $r->item_number;
            $to_export[lang('common_package_number')][] = $p_id;
            ...
            if (count($authed_locations) == 1) {
                $to_export[lang('items_safe')][] = to_quantity($r->safe, FALSE);
                $to_export[lang('items_floor')][] = to_quantity($r->floor, FALSE);
            }
            ...
            if ($include_location_columns) {
                foreach ($authed_locations as $location_id) {
                    ...
                    if (count($authed_locations) != 1) {
                        $to_export[$location_info->name . ' ' . lang('items_safe')][] = to_quantity($item_location_info->safe, false);
                        $to_export[$location_info->name . ' ' . lang('items_floor')][] = to_quantity($item_location_info->floor, false);
            ...
            if ($this->Item_attribute->has_attributes($r->item_id)) {
                foreach ($this->Item_variations->get_variations($r->item_id) as $variation_id => $variation) {
                    ...
                    $variation_label = '';
                    // $variation_safe = $this->Item_variation_location->get_location_quantity($variation_id);
                    // $variation_floor = $this->Item_variation_location->get_location_floor($variation_id);
                    ...
                    // if (count($authed_locations) == 1) {
                    //     $variation_export_row[lang('items_safe')] = $variation_safe;
                    //     $variation_export_row[lang('items_floor')] = $variation_floor;
                    // }
                    ...
                    if ($include_location_columns) {
                        foreach ($authed_locations as $location_id) {
                            ...
                            // if (count($authed_locations) != 1) {
                            //     $variation_export_row[$location_info->name . ' ' . lang('items_safe')] = $item_variation_location_info->safe ? to_quantity($item_variation_location_info->safe, false) : '';
                            //     $variation_export_row[$location_info->name . ' ' . lang('items_floor')] = $item_variation_location_info->floor ? to_quantity($item_variation_location_info->floor, false) : '';
                            // }
    ...
    private function _get_database_fields_for_import_as_array()
    {
        ...
        $fields[] = array('Name' => lang('items_safe'), 'key' => 'safe');
        $fields[] = array('Name' => lang('items_floor'), 'key' => 'floor');
        ...
        foreach ($authed_locations as $location_id) {
            ...
            if (count($authed_locations) != 1) {
                $fields[] = array('Name' => $location_info->name . ' ' . lang('items_safe'), 'key' => $location_id . '|safe');
                $fields[] = array('Name' => $location_info->name . ' ' . lang('items_floor'), 'key' => $location_id . '|floor');
        ...
        for ($i = 0; $i < $numRows - 1; $i++) {
            ...
            foreach ($fields as $field) {
                ...
                if ($field['key'] !== "") {
                    ...
                    } elseif ($field['key'] == 'safe') {
                        $safe = $this->_clean($field['key'], $columns_with_data[$key]['data'][$i]);
                    }elseif ($field['key'] == 'floor') {
                        $floor = $this->_clean($field['key'], $columns_with_data[$key]['data'][$i]);
            ...
            } elseif ($variation && $last_saved_item_id) //If we have a variation and then try to save a variation
            {
                ...
                if (count($attribute_value_pairs) > 0) {
                    ...
                    if (count($variation_attribute_value_ids) > 0) {
                        ...
                        //if ((isset($safe) && $safe !== '' && $safe !== NULL) && (isset($floor) && $floor !== '' && $floor !== NULL)) {
                            //$cur_item_variation_location_info_before_save = $this->Item_variation_location->get_info($item_variation_id);
                                
                            // $this->Item_variation_location->save_quantity($cur_item_variation_location_info_before_save->item_weight,$safe,$floor, $item_variation_id);
                            // $employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
                            ...
                            //     'trans_inventory_safe' => $safe - (float)$cur_item_variation_location_info_before_save->safe,
                            //     'trans_inventory_floor' => $floor - (float)$cur_item_variation_location_info_before_save->floor,
                            //     'location_id' => $this->Employee->get_logged_in_employee_current_location_id(),
                            //     'trans_current_safe' => $safe,
                            //     'trans_current_floor' => $floor,
                            //     'trans_type' => 'Inventory'
                            // );

                            // if (($safe - (float)$cur_item_variation_location_info_before_save->safe != 0) && ($floor - (float)$cur_item_variation_location_info_before_save->floor != 0)) {
                            //     $this->Inventory->insert($inv_data);
                            // }
                        //}
                    }
                ...
                if (!empty($item_location_data)) {
                    foreach ($item_location_data as $cur_location_id => $cur_item_location_data) {
                        ...
                        if ($item_variation_id && (isset($cur_item_location_data['safe'])) && (isset($cur_item_location_data['floor']))) {
                            // if (!$this->Item_variation_location->save_quantity($item_variation_location_info_before_save->item_weight,$cur_item_location_data['safe'],$cur_item_location_data['floor'], $item_variation_id, $cur_location_id)) {
                            //     $this->_logDbError($i + 2);
                            //     $can_commit = FALSE;
                            //     continue;
                            // }
                            ...
                            // if (($cur_item_location_data['safe'] != (float)$item_variation_location_info_before_save->safe) && ($cur_item_location_data['floor'] != (float)$item_variation_location_info_before_save->floor)) {
                            //     $inv_data = array
                            //     (
                            //         'trans_date' => date('Y-m-d H:i:s'),
                            //         'trans_items' => $item_id_to_use_for_variation,
                            ...
                            //         'trans_inventory_safe' => (float)$cur_item_location_data['safe'] - (float)$item_variation_location_info_before_save->safe,
                            //         'trans_inventory_floor' => (float)$cur_item_location_data['floor'] - (float)$item_variation_location_info_before_save->floor,
                            //         'location_id' => $cur_location_id,
                            //         'trans_current_safe' => $cur_item_location_data['safe'],
                            //         'trans_current_floor' => $cur_item_location_data['floor'],
                            //         'trans_type' => 'Inventory'
                            //     );
                            ...
                            // unset($cur_item_location_data['safe']);
                            // unset($cur_item_location_data['floor']);
                        }
            ...
            if (!empty($item_location_data)) {
                foreach ($item_location_data as $cur_location_id => $cur_item_location_data) {
                    ...
                    if ($item_id && ((isset($cur_item_location_data['safe'])) || (isset($item_data['is_service']) && $item_data['is_service']) && ((isset($cur_item_location_data['floor'])) || (isset($item_data['is_service']) && $item_data['is_service']))) ) {
                        if (!$this->Item_location->save_quantity($cur_item_location_data['safe'],$cur_item_location_data['floor'], $item_id, $cur_location_id)) {
                            $this->_logDbError($i + 2);
                            $can_commit = FALSE;
                            continue;
                        }
                        ...
                        //Only log inventory if quantity changes
                        if ((!isset($item_data['is_sevice']) || !$item_data['is_service']) && $cur_item_location_data['safe'] != (float)$item_location_before_save->safe && $cur_item_location_data['floor'] != (float)$item_location_before_save->floor) {
                            $inv_data = array
                            (
                                'trans_date' => date('Y-m-d H:i:s'),
                                ...
                                'trans_user' => $employee_id,
                                'trans_comment' => $comment,
                                'trans_inventory_safe' => (float)$cur_item_location_data['safe'] - (float)$item_location_before_save->safe,
                                'trans_inventory_floor' => (float)$cur_item_location_data['floor'] - (float)$item_location_before_save->floor,
                                'location_id' => $cur_location_id,
                                'trans_current_safe' => $cur_item_location_data['safe'],
                                'trans_current_floor' => $cur_item_location_data['floor'],
                                'trans_type' => 'Inventory'
            ...
            if (($item_id && (isset($safe) && $safe !== '') &&  (isset($floor) && $floor !== '')) || (isset($item_data['is_service']) && $item_data['is_service'])) {
                if (!$this->Item_location->save_quantity($safe,$floor, $item_id)) {
                    $this->_logDbError($i + 2);
                    $can_commit = FALSE;
                    continue;
                }
                ...
                //Only log inventory if quantity changes
                if ((!isset($item_data['is_sevice']) || !$item_data['is_service']) && $safe != (float)$item_location_before_save->safe && $floor != (float)$item_location_before_save->floor) {
                    $inv_data = array
                    (
                        'trans_date' => date('Y-m-d H:i:s'),
                        'trans_items' => isset($item_data['item_id']) ? $item_data['item_id'] : $item_id,
                        'trans_user' => $employee_id,
                        'trans_comment' => $comment,
                        'trans_inventory_safe' => $safe - (float)$item_location_before_save->safe,
                        'trans_inventory_floor' => $floor - (float)$item_location_before_save->floor,
                        'location_id' => $this->Employee->get_logged_in_employee_current_location_id(),
                        'trans_current_safe' => $safe,
                        'trans_current_floor' => $floor,
                        'trans_type' => 'Inventory'
    ...
    //Item sold type and MMJ weight details
	function get_category_sold_type_and_mmj_info($category_id)
	{			
        $cat_info = $this->Category->sold_type_and_mmj_info($category_id);
    ...
    function count_not_counted($in_stock = 0, $count_id, $offset = 0)
    {
        ...
        foreach ($items_not_counted as $item) {
            ...
            if ($this->Employee->has_module_action_permission('items', 'see_count_when_count_inventory', $this->Employee->get_logged_in_employee_info()->person_id)) {
                $tab_data_row[] = array('data' => to_quantity( $this->Item_location->get_location_quantity($item['item_id'], $count_info->location_id)), 'align' => 'center');
            }
    ...
    function edit_count_item()
    {
        ...
        if (isset($variation)) {
            ...
            if ($this->input->post('value')) {
                $this->load->model('Item_location');
                $actual_quantity = $this->Item_location->get_location_quantity($item_id, $count_info->location_id);
    ...
    function save_brand($brand_id = FALSE)
    {
        $update = $brand_id ? true : false;

        $brand_name = $this->input->post('brand_name');

        $exist = $this->Brand->check_brandname_exist($brand_name);

        if($exist)
        {
            echo json_encode(array('success' => false, 'message' => lang('items_brand_already_exists_error')));
        }
        else
        {
            if($update)
            {
                $update_success = $this->Brand->update_brand($brand_id, $brand_name);

                if($update_success)
                {
                    echo json_encode(array('success' => true, 'message' => lang('items_brand_successful_adding') . ' ' . H($category_name), 'brands' => array('brand_id' => $update_success,'brand_name' => $brand_name), 'selected' => $update_success));
                } 
                else 
                {
                    echo json_encode(array('success' => false, 'message' => lang('items_brand_successful_error')));
                }
            }
            else
            {
                $create_success = $this->Brand->create_brand($brand_name);

                if($create_success)
                {
                    echo json_encode(array('success' => true, 'message' => lang('items_brand_successful_adding') . ' ' . H($category_name), 'brands' => array('brand_id' => $create_success,'brand_name' => $brand_name), 'selected' => $create_success));
                } 
                else 
                {
                    echo json_encode(array('success' => false, 'message' => lang('items_brand_successful_error')));
    ...
    {          
        ...
        $items_list =  $this->Item->get_items_for_cron();
        // $items_list =  array(array('item_id' => '909'),array('item_id' => '910'),array('item_id' => '911'));        
        foreach($items_list as $row => $item){ 
            $store_data = array();
            $current_item = $item['item_id']; 
            $store_data['item_id'] = $current_item; 
            $store_data['mmj_weight'] = $item['mmj_weight']; 

            //if any transaction today for this item 
            $transactions = $this->Item->get_transactions_for_today($current_item);
            
            if(count($transactions) > 0){
                $previous_entry = $this->Item->get_cron_previous_entry($current_item);
                if($previous_entry){
                    // Previous day Balance as today stock (starting Safe & floor)
                    $store_data['safe'] = $previous_entry->balance_safe; 
                    $store_data['floor'] = $previous_entry->balance_floor;
                    //Day End transaction safe and floor
                    $last_transaction = array_pop($transactions);
                    $store_data['balance_safe'] = $last_transaction['trans_current_safe']; 
                    $store_data['balance_floor'] = $last_transaction['trans_current_floor']; 
                }else{
                    // Previous day Balance from day starting record
                    $first_transaction = $transactions[0];
                    $store_data['safe'] = $first_transaction['trans_current_safe']; 
                    $store_data['floor'] = $first_transaction['trans_current_floor'];
                    //Day End transaction safe and floor
                    $last_transaction = array_pop($transactions);
                    $store_data['balance_safe'] = $last_transaction['trans_current_safe']; 
                    $store_data['balance_floor'] = $last_transaction['trans_current_floor']; 
                ...
                //Today Purchases
                $store_data['purchase'] = 0;
                $purchases = $this->Item->get_purchase_order_for_today($current_item);            
                if(count($purchases) > 0){
                    $total_purchase = 0;
                    foreach ($purchases as $key => $purchase) {
                        $total_purchase = $total_purchase + $purchase['quantity_received'];
                    }
                    $store_data['purchase'] = $total_purchase;                
                }
                
                //Today Sales
                $store_data['sale'] = 0;
                $sales = $this->Item->get_sales_order_for_today($current_item);   
                if(count($sales) > 0){
                    $total_sales = 0;
                    foreach ($sales as $key => $sale) {
                        $total_sales = $total_sales + $sale['quantity_purchased'];
                    }
                    $store_data['sale'] = $total_sales;
                }
            }else{
                //if item have record in cron table
                $previous_entry = $this->Item->get_cron_previous_entry($current_item);
                if($previous_entry){
                    $store_data['safe'] = $previous_entry->balance_safe; 
                    $store_data['floor'] = $previous_entry->balance_floor;
                    $store_data['purchase'] = 0; 
                    $store_data['sale'] = 0; 
                    $store_data['balance_safe'] = $previous_entry->balance_safe; 
                    $store_data['balance_floor'] = $previous_entry->balance_floor;                  
                }else{
                    echo 'No Record in CRON';
                }   
            }
            $store_data['category_id'] = $item['category_id'];
            $this->Inventory_snapshot->insert($store_data);
        }
    }

    function labels($item_id = -1, $offset = 0)
    {
        $data['item_info'] = $this->Item->get_info($item_id);
        $this->load->view("items/labels", $data);
    }
}

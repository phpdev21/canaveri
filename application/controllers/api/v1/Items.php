    ...
    private function _populate_custom_fields($item_request,&$item_data)
    {
		...
		public function _save_item_location_data($location_data,$item_id)
		{
			...
			foreach($location_data as $location_id=>$item_location_info)
			{
				...
				if (isset($item_location_info['safe']) || isset($item_location_info['floor']) )
				{
					$item_location_data['safe'] = $item_location_info['safe'];
					$item_location_data['floor'] = $item_location_info['floor'];
					
					$cur_item_location_info = $this->Item_location->get_info($item_id,$location_id);
				
					if (($cur_item_location_info->safe != $item_location_data['safe']) || ($cur_item_location_info->floor != $item_location_data['floor']))
						{
						$inv_data = array
							(
							...
							'trans_user'=>1,
							'trans_comment'=>'API',
							'trans_inventory_safe'=>$item_location_data['safe'] - (float)$cur_item_location_info->safe,
							'trans_inventory_floor'=>$item_location_data['floor'] - (float)$cur_item_location_info->floor,
							'location_id'=>$location_id,
							'trans_current_safe' => $item_location_data['safe'],
							'trans_current_floor' => $item_location_data['floor'],
				...
				if (isset($item_location_info['variations']))
				{
					foreach($item_location_info['variations'] as $variation_data)
					{
						...
						// if(isset($variation_data['safe']) || isset($variation_data['floor']))
						// {
						// 	$item_variation_location_data['safe'] = $variation_data['safe'];
						// 	$item_variation_location_data['floor'] = $variation_data['floor'];
							
						// 	$cur_item_var_location_info = $this->Item_variation_location->get_info($item_variation_id,$location_id);
				
						// 	if ($cur_item_var_location_info->safe != $item_variation_location_data['safe'] || $cur_item_var_location_info->floor != $item_variation_location_data['floor'])
						// 		{
						// 		$inv_data = array
						// 			(
						// 			'trans_date'=>date('Y-m-d H:i:s'),
						...
						// 			'trans_user'=>1,
						// 			'trans_comment'=>'API',
						// 			'trans_inventory_safe'=>$item_variation_location_data['safe'] - $cur_item_var_location_info->safe,
						// 			'trans_inventory_floor'=>$item_variation_location_data['floor'] - $cur_item_var_location_info->floor,
						// 			'location_id'=>$location_id,
						// 			'trans_current_safe' => $item_variation_location_data['safe'],
						// 			'trans_current_floor' => $item_variation_location_data['floor'],
						// 			'trans_type' => 'Inventory'	
						// 		);
						// 		$this->load->model('Inventory');
						// 		$this->Inventory->insert($inv_data);

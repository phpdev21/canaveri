...
{    
    function __construct()
    {
        ...
        $this->load->model('Supplier_brands');
        ...
        else
        {
            if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
            {
                ...
                for($k = 2;$k<=$num_rows; $k++)
                {
                    ...
                    $state_license_number = $sheet->getCellByColumnAndRow(13, $k);
                    if (!$state_license_number) {
                        $state_license_number = NULL;
                    }

                    $state_regulatory_agency_number = $sheet->getCellByColumnAndRow(14, $k);
                    if (!$state_regulatory_agency_number) {
                        $state_regulatory_agency_number = NULL;
                    }
                    ...
                    $supplier_data=array(
                        ...
                        'state_regulatory_agency_number' => $state_regulatory_agency_number
                    );
    ...
    function excel_export() {
        ...
        foreach ($data as $r) {
            $row = array(
                ...
                $r->comments,
                $r->account_number,
                $r->state_license_number,
                $r->state_regulatory_agency_number,
            );
    ...
    */
    function view($supplier_id=-1, $redirect = 0)
    {
        ...
        $data['selected_brands']= ($supplier_id == -1)? []: $this->Supplier_brands->getSupplierBrands($supplier_id);
    ...
    */
    ...
    {        
        ...
        $supplier_data=array(
            ...
            'state_license_number'=> $this->input->post('state_license_number') ? $this->input->post('state_license_number') : NULL,
            'state_regulatory_agency_number'=> $this->input->post('state_regulatory_agency_number') ? $this->input->post('state_regulatory_agency_number') : NULL,
            'supplier_type' => $this->input->post('supplier_type') ? $this->input->post('supplier_type') : NULL,
        );
        ...
        {            
            ...
            else //previous supplier
            {
             ...
             //=======================================
             $brand_list = $this->input->post('brands');
             foreach($brand_list as $brand_name) 
             {
                if($brand_name != '')
                {
                    $supplier_brand_id = $this->Supplier_brands->check_supplier_brand_exist($supplier_id, $brand_name);
                    if($supplier_brand_id  === '')
                    {
                        $this->Supplier_brands->insert_supplier_brand($supplier_id, $brand_name);
                    }else{
                        $this->Supplier_brands->update_supplier_brand($supplier_brand_id, $brand_name);
                    }
                }
             }
             $delete_brand_list = $this->input->post('supplier_brands_to_delete');
             foreach($delete_brand_list as $supplier_brand_id) 
             {
                 $this->Supplier_brands->delete_supplier_brand($supplier_brand_id);
             }
             //=======================================
    ...
    */
    ...
    function getSupplierBrands()
    {
        $supplier_id = (int)$this->input->get('supplierId');
        $supplier_brands = $this->Supplier_brands->getSupplierBrands($supplier_id);
        echo json_encode($supplier_brands);
    }
}
?>
...
class Sales extends Secure_area
{
	...
	function complete()
	{
		...
		{
			$del_info = $this->cart->get_delivery_person_info();
		    $data['delivery_person_info'] = $del_info;
			$data['full_delivery_address'] = $del_info['address_1']." ".$del_info['address_2']." ".$del_info['city'].", ".$del_info['state']." ".$del_info['zip'];
	...
	{	
		...
		$last_sale_id = $this->db->select('sale_id')->from('sales')->order_by('sale_id',"desc")->limit(1)->get()->result();
		if(empty($data)){
			$last_sale_id = $data[0]->sale_id;
		}else{
			$last_sale_id = "none";
		}
		...
		$data['modes']= $modes;
		$data['last_sale_id'] =  $last_sale_id;
  ...
  function cancel_sale()
  {
	...
	function get_attributes_values() {
		...
		{
			$data = $this->_get_shared_data();			
			$this->load->view("sales/labels", $data);
		}

		//Print Label
		function labelSingle($idSlug = '',$id = '')
		{
			$data = $this->_get_shared_data();	
			...
			foreach($data['cart_items'] as $key=>$val){
				if($idSlug == 'item_kit_id'){
					if(!isset($val->item_kit_id) || $val->item_kit_id != $id){
						unset($data['cart_items'][$key]);
					}
				}else if($idSlug == 'item_id'){
					if(!isset($val->item_id) || $val->item_id != $id){
						unset($data['cart_items'][$key]);
					}
				}
			}
			...
			// unset($data['cart_items'][0]);
			
				
			$this->load->view("sales/labels", $data);
		}

	function helox_order_payments() {
		$order_id = $_POST['order_id'];
		$price = $_POST['price'];
		$mobile_number = $_POST['mobile_number'];
		$call_backurl = base_url()."index.php/sales/sale_helox_orders/".$order_id;
		$curl = curl_init();
		curl_setopt_array($curl, array(
		CURLOPT_URL => 'https://api.monarch.is/v1/mobile/helox/pos',
		...
		CURLOPT_HTTPHEADER => array(
			'X-API-KEY: ca1fa546-bf8d-44ab-8cc4-de5cf227b771',
			'X-APP-ID: b508dde4',
	...
	function helox_order_payment_status() {
		$curl = curl_init();
		curl_setopt_array($curl, array(
		CURLOPT_URL => 'https://api.monarch.is/v1/mobile/helox/pos/paymentStatus/'.$_POST['order_id'],
		CURLOPT_RETURNTRANSFER => true,
		CURLOPT_ENCODING => '',
		CURLOPT_MAXREDIRS => 10,
		CURLOPT_TIMEOUT => 0,
	...
	function sale_helox_orders($order_id) {

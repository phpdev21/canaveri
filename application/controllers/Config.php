...
class Config extends Secure_area 
{
	...
	function index()
	{
		...
		$data['receiving_payment_options']=array(
			lang('common_cash') => lang('common_cash'),
		...
		foreach($this->Appconfig->get_additional_payment_types() as $additional_payment_type)
		{
			...
			$data['receiving_payment_options'][$additional_payment_type] = $additional_payment_type;
		}
    ...
    $data['item_lookup_order'] = unserialize($this->config->item('item_lookup_order'));
		...
		$data['grams_per_pound'] = $this->config->item('grams_per_pound');
		$data['ounce_per_pound'] = $this->config->item('ounce_per_pound');
		$data['categories'][''] = lang('common_select_category');
		$data['section_purchase_settings'] = $this->config->item('section_purchase_settings');
		$data['helox_payment_prefix'] = $this->config->item('helox_payment_prefix');
	...
	function save()
	{
		$category_arr = [];
        $weight_arr = [];
		if(isset($_POST['category_limit_name']) && isset($_POST['category_limit_value'])){
			foreach($_POST['category_limit_name'] as $category_limit_name){   
				array_push($category_arr ,$category_limit_name);
			}
			foreach($_POST['category_limit_value'] as $category_limit_value){   
				array_push($weight_arr ,$category_limit_value);
			}
			$a1=$category_arr;
			$a2=$weight_arr;
			$final= [];
			...
			foreach($a1 as $value1){
				foreach($a2 as $value2){
					if(!empty($final)){
						if(!in_array($value1,$final[$count_]) && !in_array($value2,$final[$count_])){
							$combine_array =[$value1,$value2];
							array_push($final,$combine_array);
							$count_++;
						}
					}else{
						if(!in_array($value1,$final) && !in_array($value2,$final)){
							$combine_array =[$value1,$value2];
							array_push($final,$combine_array);
						}
					}
			...
			$this->Appconfig->save('section_purchase_settings',json_encode($final));
			$this->Appconfig->save('helox_payment_prefix',$_POST['helox_payment_prefix']);
			//echo json_encode(array('success'=>true,'message'=>lang('common_saved_successfully')));
		}else{
			$this->Appconfig->save('section_purchase_settings',json_encode([]));	
		}
      ...
      curl_close($ch);		
		...
		'metrc_api_key'=>$this->input->post('metrc_api_key'),
		...
		'grams_per_pound' => $this->input->post('grams_per_pound') ? $this->input->post('grams_per_pound') : '453.5923', //default 
		'ounce_per_pound' => $this->input->post('ounce_per_pound') ? $this->input->post('ounce_per_pound') : '16', //default 
		...
		//New CAS.420 configurations
		'cas_store_type' => $this->input->post('cas_store_type'),
		'cas_company_title' => $this->input->post('cas_company_title'),
		'cas_license_number' => $this->input->post('cas_license_number'),
	);

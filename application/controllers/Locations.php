...
class Locations extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function does_have_valid_number_of_locations_for_an_additional_location($email)
	{
		...
		return TRUE; //($authenticated_locations_count >= ($current_location_count + 1));
	}
	...
	function save($location_id=-1)
	{
		...
		if ($location_id == -1)
		{
			...
			$this->load->helper('demo');
			if (!is_on_demo_host() && (!$this->does_have_valid_number_of_locations_for_an_additional_location($purchase_email)))
			{

...
{	
	...
	function export_recv($recv_id)
	{
		...
		$header_row[] = lang('common_batch_numbers');
		...
		foreach($this->Receiving->get_receiving_items($recv_id)->result() as $item)
		{
			...
			if (isset($additional_item_numbers[$item->item_id]) && count($additional_item_numbers[$item->item_id]) > 0) {
                foreach ($additional_item_numbers[$item->item_id] as $item_num) {
                    if ( $item_info->item_number) {
                        $item_info->item_number .= "|";
                    }
					$item_info->item_number .= $item_num;
                }
            }
			...
			$row[] = $item->serialnumber ?  $item->serialnumber : '';
			if(!$this->config->item('hide_size_field'))
			{

...
class Customers extends Person_controller
{
    ...
    function save($customer_id = -1)
    {
        ...
        $customer_data = array(
            ...
            'rx_number' => $this->input->post('rx_number'),
            'rx_exp_date' => $this->input->post('rx_exp_date'),
            'med_account_number' => $this->input->post('med_account_number')
        );
    ...
    private function _get_database_fields_for_import_as_array()
    {
        ...
        $fields[] = array('Name' => lang('customers_customer_type'), 'key' => 'customer_type');
        $fields[] = array('Name' => lang('customers_drivers_license_number'), 'key' => 'license_number');
        $fields[] = array('Name' => lang('customers_drivers_license_state'), 'key' => 'license_state');
        $fields[] = array('Name' => lang('customers_drivers_license_exp_date'), 'key' => 'license_exp_date');
        $fields[] = array('Name' => lang('customers_customer_dob'), 'key' => 'date_of_birth');
        $fields[] = array('Name' => lang('customers_medicinal_card_number'), 'key' => 'rx_number');
        $fields[] = array('Name' => lang('customers_medicinal_card_exp_date'), 'key' => 'rx_exp_date');
        $fields[] = array('Name' => lang('customers_medicinal_account_number'), 'key' => 'med_account_number');
        $this->lang->load('locations');
    ...
    private function _clean($key, $value, $row = NULL)
    {
        ...
        if ($key == 'license_state') {
            if (!$value) {
                return NULL;
            }
            return $value;
        }
        if ($key == 'license_exp_date') {
            if (!$value) {
                return NULL;
            }
            return $value;

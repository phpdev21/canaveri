...
function create_breadcrumb()
{
	...
	elseif($ci->uri->segment(1) == 'state_reporting')
	{
		if ($ci->uri->segment(3) == NULL) //Main page
		{
			$reports_home_link =create_current_page_url(lang('module_state_reporting'));
		}
		else
		{
			$reports_home_link = '<a tabindex = "-1" href="'.site_url('state_reporting').'">'.lang('module_state_reporting').'</a>';
		}
		
		$return.=$reports_home_link;
		if($ci->uri->segment(3) == 'purchase_report_grower')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_purchase_report_grower'));
		}
		elseif($ci->uri->segment(3) == 'purchase_report_processor')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_purchase_report_processor'));
		}
		elseif($ci->uri->segment(3) == 'items_sold_report')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_items_sold_report'));
		}
		elseif($ci->uri->segment(3) == 'inventory_report')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_inventory_report'));
		}
		elseif($ci->uri->segment(3) == 'inventory_additions_and_subtractions')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_inventory_addition_subtraction'));
		}
		elseif($ci->uri->segment(3) == 'detailed_inventory_report')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_detailed_inventory_report'));
		}
		elseif($ci->uri->segment(3) == 'detailed_item_sold_report')
		{
			$return.=create_report_breadcrumb(lang('breadcrumb_detailed_item_sold_report'));
		}
	}

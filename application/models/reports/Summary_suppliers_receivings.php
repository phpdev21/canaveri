...
class Summary_suppliers_receivings extends Report
{
	...
	public function getDataColumns()
	{
		...
		$columns[] = array('data'=>lang('reports_supplier_state_license_number'), 'align'=> 'left');
        $columns[] = array('data'=>lang('reports_supplier_state_regulatory_agency_number'), 'align'=> 'left');
	...
	public function getOutputData()
	{
		...
		{				
			...
			foreach($report_data as $row)
			{
                ...
                $data_row[] = array('data'=>$row['state_license_number'], 'align' => 'left');
                $data_row[] = array('data'=>$row['state_regulatory_agency_number'], 'align' => 'left');
	...
	{
		$this->db->select('CONCAT(company_name, " (",first_name, ")") as supplier, state_license_number, state_regulatory_agency_number, sum('.$this->db->dbprefix('receivings').'.subtotal) as subtotal, sum('.$this->db->dbprefix('receivings').'.total) as total, sum('.$this->db->dbprefix('receivings').'.tax) as tax, sum('.$this->db->dbprefix('receivings').'.profit) as profit', false);

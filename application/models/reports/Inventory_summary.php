...
class Inventory_summary extends Report
{
	...
	public function getOutputData()
	{
		...
		{
			$data_row = array();
			$inven = $this->getIventioryAsPerSale($row['item_id']);
			$safe = $inven['trans_current_safe'] + $inven['trans_inventory_safe'];
			$floor =  $inven['trans_current_floor'] - $inven['trans_inventory_floor'];
			...
			$data_row[] = array('data'=>to_quantity($safe), 'align'=> 'left');
			$data_row[] = array('data'=>to_quantity($floor), 'align'=> 'left');
			$data_row[] = array('data'=>to_currency($row['cost_price']*($safe+$floor)), 'align'=> 'right');
			$data_row[] = array('data'=>to_currency($row['unit_price']*($safe+$floor)), 'align'=> 'right');
		...
		//echo '<pre>';print_r($this->getSummaryData());exit;
	...
	public function getData()
	{
		...
		if ($this->params['category_id'])
		{
		    ...
		    $query = 'SELECT X.*,Z.name
					...
					SUM( DISTINCT(phppos_location_item_variations.safe)) as floor, 
	...
	private function dataQuery()
	{
		...
		$safe_query = 'SUM('.$safe_quantity_query.')';
		$floor_query = 'SUM('.$floor_quantity_query.')';
		...
		'.$safe_query.' as safe, '.$floor_query.' as floor,
		...
		{
			$this->db->having($safe_query.' > 0 or '.$floor_query.' > 0');
		}

		if ($this->params['inventory'] == 'out_of_stock')
		{
			$this->db->having($safe_query.' <= 0 and '.$floor_query.' <= 0');
		}
	...
	public function getSummaryData()
	{
		...
		$location_item_variations_safe_col = $this->db->dbprefix('location_item_variations').'.safe';
		$location_items_safe_col = $this->db->dbprefix('location_items').'.safe';

		$location_item_variations_floor_col = $this->db->dbprefix('location_item_variations').'.floor';
		$location_items_floor_col = $this->db->dbprefix('location_items').'.floor';
		
		
		$full_sum_query = 'COALESCE(SUM('.$location_item_variations_safe_col.')+SUM('.$location_item_variations_floor_col.'),SUM('.$location_items_safe_col.')+SUM('.$location_items_floor_col.'),0)';
		$quantity_query = 'COALESCE('.$location_item_variations_safe_col.'+'.$location_item_variations_floor_col.','.$location_items_safe_col.'+'.$location_items_floor_col.',0)';			
		...
		if ($this->params['inventory'] == 'in_stock')
		{
			$in_stock = 'AND ifnull(safe, 0) > 0 OR ifnull(floor, 0) > 0';
		}
		$out_stock = '';
		if ($this->params['inventory'] == 'out_of_stock')
		{
			$out_stock = 'AND ifnull(safe, 0) <= 0 AND ifnull(floor, 0) <= 0';
		}
		...
		else
		{
		    ...
		    $query = 'select 
                ...
                ifnull(q.safe,0) + ifnull(q.floor,0) + ifnull(v.varaition_safe,0) + ifnull(v.varaition_floor,0) as total_items_in_inventory,
                
                (ifnull(q.safe,0) + ifnull(q.floor,0) + ifnull(v.varaition_safe,0) + ifnull(v.varaition_floor,0)) * cost_price as inventory_total,
                (ifnull(q.safe,0) + ifnull(q.floor,0) + ifnull(v.varaition_safe,0) + ifnull(v.varaition_floor,0)) * unit_price  as inventory_sale_total
                ,
                q_unit,
                v_unit,
                q_cost_price
				...
				SELECT item_id , sum(safe) varaition_safe
				, sum(floor) varaition_floor   
	...
	function getIventioryAsPerSale($item_id){

		$this->db->select("inventory.trans_current_safe,inventory.trans_current_floor,inventory.trans_inventory_safe,inventory.trans_inventory_floor");
		$this->db->from('items');
		...
		$inventory_as_per_date = end($this->db->get()->result_array());
		 return $inventory_as_per_date;
	}
}
?>
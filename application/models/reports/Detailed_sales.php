...
class Detailed_sales extends Report
{
	...
	function getOutputData()
	{
		...
		foreach($this->params['export_excel'] == 1 && isset($report_data['summary']) ? $report_data['summary']:$report_data as $key=>$row)
		{
			...
			$summary_data_row[] = array('data'=>$row['sold_varition'], 'align'=>'left');
			...
			$summary_data_row[] = array('data'=>$row['rx_number'], 'align'=>'right');
			$summary_data_row[] = array('data'=>$row['each_sold_items'], 'align'=>'right');
			$summary_data_row[] = array('data'=>$row['thc'], 'align'=>'right');
			$summary_data_row[] = array('data'=>$row['cbd'], 'align'=>'right');
			$summary_data_row[] = array('data'=>$row['terp'], 'align'=>'right');
			$summary_data_row[] = array('data'=>$row['uom'], 'align'=>'right');
			...
			if($this->params['export_excel'] == 1)
			{
				...
				{	
					...
					$additional_item_numbers = $this->Additional_item_numbers->get_all(false);
						if (isset($additional_item_numbers[$drow['item_id']]) && count($additional_item_numbers[$drow['item_id']]) > 0) {
							foreach ($additional_item_numbers[$drow['item_id']] as $item_num) {
								if ($drow['item_number']) {
									$itemnumbrs.= "|";
								}
								$itemnumbrs .= $item_num;
					...
					$details_data_row[] = array('data'=>$itemnumbrs, 'align'=>'left');
					...
					$details_data_row[] = array('data'=>$drow['sold_varition'], 'align'=>'left');
	...
	public function getDataColumns()
	{
		...
		$return['summary'][] = array('data'=>lang('reports_sale_variation'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('reports_register'), 'align'=> 'left');
		$return
		['summary'][] = array('data'=>lang('common_items_purchased'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('reports_sold_by'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('reports_sold_to_customer'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('customers_medicinal_card_number'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('each_item_purchased_in_sale'), 'align'=> 'left');	
		$return['summary'][] = array('data'=>lang('thc'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('cbd'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('terp'), 'align'=> 'left');
		$return['summary'][] = array('data'=>lang('common_unit_of_measure'), 'align'=> 'left');
	...
	{		
		...
		if (isset($this->params['export_excel']) && !$this->params['export_excel'])
		{
			...
			$sale_ids = array();
			$table_all_data = $this->db->get()->result_array(); 
			foreach($table_all_data as $table_data)
			{

				$sale_ids[] =  $table_data['saleid'];
			}
			$result_vartions = $this->get_report_details($sale_ids,1); 
			foreach($result_vartions as $sale_item_row)
			{
				$sold_varition = $this->Item_variations->get_item_variation_attribute_value($sale_item_row['item_variation_id']);
				if($sold_varition !=""){
					$table_data['sold_varition'] = $sold_varition;
				}else{
					$table_data['sold_varition'] = "";
				}
				array_push($table_data_new,$table_data);
			}
			foreach($table_all_data as $table_data)
			{ 
				$this->db->select("items.name,items.custom_field_1_value");
				$this->db->from('items');
				$this->db->join('sales_items', 'items.item_id = sales_items.item_id', 'left');
				$this->db->where('sales_items.sale_id', $table_data['saleid']);
				$total_items_solds = $this->db->get()->result_array();
				$itmesin_sold = '';
				if(!empty($total_items_solds)){
					foreach ($total_items_solds as $key => $total_items_sold) {
						$table_data['thc'] =  $total_items_sold['custom_field_1_value'];
						$table_data['cbd'] =  $total_items_sold['custom_field_2_value'];
						$table_data['terp'] =  $total_items_sold['custom_field_3_value'];
						if ($total_items_sold['name']) {
							if ($key !="") {
								$itmesin_sold .= ",";
							}
						}
						$itmesin_sold .= $total_items_sold['name'];
					} 
					$table_data['each_sold_items'] = $itmesin_sold;
					array_push($table_data_new,$table_data);
				}
		...
		if (isset($this->params['export_excel']) && $this->params['export_excel'] == 1)
		{
			...
			foreach($this->db->get()->result_array() as $sale_summary_row)
			{
				...
				$this->db->select("items.name,items.custom_field_1_value,items.custom_field_2_value,items.custom_field_3_value,items.custom_field_5_value");
				$this->db->from('items');
				$this->db->join('sales_items', 'items.item_id = sales_items.item_id', 'right');
				$this->db->where('sales_items.sale_id', $sale_summary_row['sale_id']);
				$total_items_solds = $this->db->get()->result_array();
				$itmesin_sold = '';
				if(!empty($total_items_solds)){
					foreach ($total_items_solds as $key => $total_items_sold) {
						$data['summary'][$sale_summary_row['sale_id']]['thc'] =  $total_items_sold['custom_field_1_value'];
						$data['summary'][$sale_summary_row['sale_id']]['cbd'] =  $total_items_sold['custom_field_2_value'];
						$data['summary'][$sale_summary_row['sale_id']]['terp'] =  $total_items_sold['custom_field_3_value'];
						$data['summary'][$sale_summary_row['sale_id']]['uom'] =  $total_items_sold['custom_field_5_value'];
						if ($total_items_sold['name']) {
							if ($key !="") {
								$itmesin_sold .= ",";
							}
					...
					$data['summary'][$sale_summary_row['sale_id']]['each_sold_items'] = $itmesin_sold;
				}
			}
			$sale_ids = array();
			...
			foreach($result as $sale_item_row)
			{
				$sold_varition = $this->Item_variations->get_item_variation_attribute_value($sale_item_row['item_variation_id']);
				if($sold_varition !=""){
					$sale_item_row['sold_varition'] = $sold_varition;
				}else{
					$sale_item_row['sold_varition'] = "";
				}
				$p_id = '';
				$product_ids = json_decode($sale_item_row['item_product_id']);
				...
				$sale_item_row['item_product_id'] = $p_id;
	...
	function get_details_data_columns_sales()
	{
		...
		$details[] = array('data'=>lang('common_source_package_number'), 'align'=> 'left');
		//$details[] = array('data'=>lang('common_package_number'), 'align'=> 'left');
		...
		$details[] = array('data'=>lang('reports_sale_variation'), 'align'=> 'left');	
		$details[] = array('data'=>lang('common_package_number'), 'align'=> 'left');
	...
	//	$details[] = array('data'=>lang('customers_medicinal_account_number'), 'align'=> 'right');

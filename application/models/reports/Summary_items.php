...
class Summary_items extends Report
{
	...
	{		
		...
		$columns[] = array('data'=>lang('reports_safe'), 'align'=> 'left');
		$columns[] = array('data'=>lang('reports_floor'), 'align'=> 'left');
	...
	public function getOutputData()
	{
		...
		{				
			...
			foreach($report_data as $row)
			{
				...
				$data_row[] = array('data'=>to_quantity($row['safe']), 'align' => 'left');
				$data_row[] = array('data'=>to_quantity($row['floor']), 'align' => 'left');
	...
	{		
		...
		if (isset($this->params['group_by_variation']) && $this->params['group_by_variation'])
		{			
			$this->db->select('item_variations.id as item_variation_id,SUM(safe) as safe,SUM(floor) as floor', FALSE);
		...
		{
			$this->db->select('items.item_id,SUM(safe) as safe,SUM(floor) as floor', FALSE);
		...
		{
			$quantities_indexed_by_id[$quan_row[$primary_key_column]]['safe'] = $quan_row['safe'];
			$quantities_indexed_by_id[$quan_row[$primary_key_column]]['floor'] = $quan_row['floor'];
		}
		
		for($k=0;$k<count($items_sales_data);$k++)
		{
			$items_sales_data[$k]['safe'] = $quantities_indexed_by_id[$items_sales_data[$k][$primary_key_column]]['safe'];
			$items_sales_data[$k]['floor'] = $quantities_indexed_by_id[$items_sales_data[$k][$primary_key_column]]['floor'];
		}
		...
		if (isset($this->params['items_to_show']) && ($this->params['items_to_show'] == 'items_without_sales'))
		{
			...
			{
				$items_in_report_without_sales[$k]['safe'] = $quantities_indexed_by_id[$items_in_report_without_sales[$k][$primary_key_column]]['safe'];
				$items_in_report_without_sales[$k]['floor'] = $quantities_indexed_by_id[$items_in_report_without_sales[$k][$primary_key_column]]['floor'];
			}			
		}
	...
	public function getSummaryData()
	{
		...
		$damaged_query = $this->db->query('SELECT SUM(phppos_damaged_items_log.damaged_safe)+SUM(phppos_damaged_items_log.damaged_floor) as damaged_qty FROM phppos_damaged_items_log WHERE location_id in ('.$location_ids_string.') and damaged_date BETWEEN '.$this->db->escape($this->params['start_date']).' and '.$this->db->escape($this->params['end_date']))->row();

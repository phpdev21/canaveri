...
class Item_location extends CI_Model
{
	...
	function save_quantity($safe,$floor, $item_id, $location_id=false)
	{
		if(!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id();
		...
		$sql = 'INSERT INTO '.$this->db->dbprefix('location_items'). ' (safe, floor, item_id, location_id)'
		    . ' VALUES (?, ?, ?,?)'
			. ' ON DUPLICATE KEY UPDATE safe = ?,floor = ?'; 
		return $this->db->query($sql, array($safe, $floor, $item_id, $location_id,$safe,$floor));		
	}
	...
	*/
	...
	function get_info($item_id,$location=false, $can_cache = false)
	{
		...
		else
		{
			...
			$fields = array('location_id','item_id','location','cost_price','unit_price','promo_price','start_date','end_date','quantity','safe','floor','reorder_level','override_default_tax','tax_class_id','replenish_level');			
	...
	*/
	...
	function get_location_floor($item_id,$location=false)
	{
		if(!$location)
		{
			$location= $this->Employee->get_logged_in_employee_current_location_id();
	...
	function get_location_floor_quantity($item_id,$location=false)
	{
		if(!$location)
		{
			$location= $this->Employee->get_logged_in_employee_current_location_id();

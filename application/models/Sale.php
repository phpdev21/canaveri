	...
	function get_payment_sales_total_for_shift($shift_start, $shift_end,$payment_type)
  {
	...
	function _get_all_sale_payments($sale_ids)
	{
		...
		if (count($sale_ids) > 0)
		{
			...
			$check_payment_types = implode(',', array_map('add_quotes_and_escape', get_all_language_values_for_key('common_helox')));
			...
			if (property_exists($item,'item_id'))
			{
				...
				if (!$cart->is_ecommerce && $suspended < 2)
				{
					...
					{	
						...
						if(!$cur_item_info->is_service && $cur_item_location_info->floor > $reorder_level)
						{
							$stock_recorder_check=true;
						}
				
						//checks if the quantity is greater than 0
						if(!$this->Location->get_info_for_key('stock_alerts_just_order_level',$cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id()) && !$cur_item_info->is_service && $cur_item_location_info->floor > 0)
						{
						...
						{
							$cur_item_location_info->floor = ($cur_item_location_info->floor !== '') ? $cur_item_location_info->floor : 0;
							//Calculate total grams by attribute value
							$attribute_value = (float)$this->Item_variations->get_item_variation_attribute_value($item->variation_id);
							$sold_quantity = ( $item->quantity * ( $item->quantity_unit_quantity !== NULL ? $item->quantity_unit_quantity : 1) ) - $item->damaged_qty;
							$new_floor_value = $cur_item_location_info->floor -  ( (int)$sold_quantity * $attribute_value);
							$this->Item_location->save_quantity($cur_item_location_info->safe,$new_floor_value , $item->item_id,$cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id());			
						...
						if($out_of_stock_check && $cur_item_location_info->floor <= 0)
						{
							$message= $cur_item_info->name.' '.$this->Item_variations->get_variation_name($item->variation_id).' '.lang('sales_is_out_stock').' '.to_quantity($cur_item_location_info->floor);
						...
						else if($stock_recorder_check && ($cur_item_location_info->floor <= $reorder_level))
						{
							$message= $cur_item_info->name.' '.$this->Item_variations->get_variation_name($item->variation_id).' '.lang('sales_hits_reorder_level').' '.to_quantity($cur_item_location_info->floor);
							if ($cur_item_info->item_id)
							{
								$message.="\n";
								$message.= lang('common_item_id').": ".$cur_item_info->item_id;
						...
						if (!$cur_item_info->is_service)
						{
							...
							//Attribute value for calculation
							$attribute_value = (float)$this->Item_variations->get_item_variation_attribute_value($item->variation_id);
							...
							(
								...
								'trans_inventory_safe'=>0,
								'trans_inventory_floor'=> ( (int)$qty_buy * $attribute_value ),
								'location_id' => $cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id(),
								'trans_current_safe' => $cur_item_location_info->safe, 
								'trans_current_floor' => $cur_item_location_info->floor, 
								'trans_type' => 'Sale'
							);
					...
					else
					{
						...
						{
							$cur_item_location_info->floor = $cur_item_location_info->floor !== '' ? $cur_item_location_info->floor : 0;
							$this->Item_location->save_quantity($cur_item_location_info->safe,$cur_item_location_info->floor - ($item->quantity*($item->quantity_unit_quantity !== NULL ? $item->quantity_unit_quantity : 1)) - ($item->damaged_qty*($item->quantity_unit_quantity !== NULL ? $item->quantity_unit_quantity : 1)), $item->item_id,$cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id());
						}
						...
						if($out_of_stock_check && $cur_item_location_info->floor <= 0)
						{
							$message= $cur_item_info->name.' '.lang('sales_is_out_stock').' '.to_quantity($cur_item_location_info->floor);
							if ($cur_item_info->item_id)
							{
								$message.="\n";
								$message.= lang('common_item_id').": ".$cur_item_info->item_id;
						...
						{
							$message= $cur_item_info->name.' '.lang('sales_hits_reorder_level').' '.to_quantity($cur_item_location_info->floor);
							if ($cur_item_info->item_id)
							{
								$message.="\n";
								$message.= lang('common_item_id').": ".$cur_item_info->item_id;
			...
			else
			{
				...
				foreach($this->Item_kit_items->get_info($item->item_kit_id) as $item_kit_item)
				{
					...
					if ($item_kit_item->item_variation_id)
					{
						...
						{
							$cur_item_location_info->floor = $cur_item_location_info->floor !== '' ? $cur_item_location_info->floor : 0;								
							$this->Item_location->save_quantity($cur_item_location_info->safe,$cur_item_location_info->floor - ($item->quantity * $item_kit_item->quantity),$item_kit_item->item_id,$cart->location_id ? $cart->location_id : $this->Employee->get_logged_in_employee_current_location_id());
						...
						if($out_of_stock_check && $cur_item_location_info->floor <= 0)
						{
							$message= $cur_item_info->name.' '.$this->Item_variations->get_variation_name($item_kit_item->item_variation_id).' '.lang('sales_is_out_stock').' '.to_quantity($cur_item_location_info->floor);
							if ($cur_item_info->item_id)
							{
								$message.="\n";
								$message.= lang('common_item_id').": ".$cur_item_info->item_id;
						...
						{
							$message= $cur_item_info->name.' '.$this->Item_variations->get_variation_name($item_kit_item->item_variation_id).' '.lang('sales_hits_reorder_level').' '.to_quantity($cur_item_location_info->floor);
							if ($cur_item_info->item_id)
							{
								$message.="\n";
								$message.= lang('common_item_id').": ".$cur_item_info->item_id;
					...
					else
					{
						...
						if (!$cart->is_ecommerce && $suspended < 2)
						{
							...
							//checks if the quantity is out of stock
							if($out_of_stock_check && !$cur_item_info->is_service && $cur_item_location_info->floor <= 0)
							{
								$message= $cur_item_info->name.' '.lang('sales_is_out_stock').' '.to_quantity($cur_item_location_info->floor);
								if ($cur_item_info->item_id)
								{
	...
	function delete($sale_id, $all_data = false)
	{
		...
		if ($suspended < 2)
		{
			...
			foreach($this->db->get()->result_array() as $sale_item_row)
			{
				...
				if (!$cur_item_info->is_service)
				{					
					$cur_item_safe = $this->Item_location->get_location_quantity($sale_item_row['item_id'], $sale_location_id);
					$cur_item_floor = $this->Item_location->get_location_floor($sale_item_row['item_id'], $sale_location_id);
					//Update stock quantity
					$this->Item_location->save_quantity($cur_item_safe,$cur_item_floor + ($sale_item_row['quantity_purchased']*($sale_item_row['unit_quantity'] !== NULL ? $sale_item_row['unit_quantity'] : 1)) + ($sale_item_row['damaged_qty']*($sale_item_row['unit_quantity'] !== NULL ? $sale_item_row['unit_quantity'] : 1)),$sale_item_row['item_id'], $sale_location_id);
					...
					(
						...
						'trans_comment'=>$sale_remarks,
						'trans_inventory_safe'=>0,
						'trans_inventory_floor'=>($sale_item_row['quantity_purchased']*($sale_item_row['unit_quantity'] !== NULL ? $sale_item_row['unit_quantity'] : 1)) + ($sale_item_row['damaged_qty']*($sale_item_row['unit_quantity'] !== NULL ? $sale_item_row['unit_quantity'] : 1)),
						'item_variation_id' => $sale_item_row['item_variation_id'] ? $sale_item_row['item_variation_id'] : NULL,
						'trans_current_safe' => $cur_item_safe,
						'trans_current_floor' => $cur_item_floor + ($sale_item_row['quantity_purchased']*($sale_item_row['unit_quantity'] !== NULL ? $sale_item_row['unit_quantity'] : 1)) + ($sale_item_row['damaged_qty']*($sale_item_row['unit_quantity'] !== NULL ? $sale_item_row['unit_quantity'] : 1)),
						'trans_type' => 'Sale'
					);
					$this->Inventory->insert($inv_data);
				}
		...
		{		
			...
			foreach($this->db->get()->result_array() as $sale_item_kit_row)
			{
				foreach($this->Item_kit_items->get_info($sale_item_kit_row['item_kit_id']) as $item_kit_item)
				{
					...
					if (!$cur_item_info->is_service)
					{
						$cur_item_location_info->safe = $cur_item_location_info->safe !== '' ? $cur_item_location_info->safe : 0;
						$cur_item_location_info->floor = $cur_item_location_info->floor !== '' ? $cur_item_location_info->floor : 0;
					
						$this->Item_location->save_quantity($cur_item_location_info->safe,$cur_item_location_info->floor + ($sale_item_kit_row['quantity_purchased'] * $item_kit_item->quantity),$item_kit_item->item_id, $sale_location_id);
						...
						(
							...
							'trans_comment'=>$sale_remarks,
							'trans_inventory_safe'=>0,
							'trans_inventory_floor'=>$sale_item_kit_row['quantity_purchased'] * $item_kit_item->quantity,
							'trans_current_safe' => $cur_item_location_info->safe,
							'trans_current_floor' => $cur_item_location_info->floor + ($sale_item_kit_row['quantity_purchased'] * $item_kit_item->quantity),
							'trans_type' => 'Sale'
						);
						$this->Inventory->insert($inv_data);
					}				
				...
				foreach($this->Item_kit_items->get_info($sale_item_kit_row['item_kit_id']) as $item_kit_item)
				{
					...
					if (!$cur_item_info->is_service && $cur_item_location_info->quantity !== NULL)
					{
						...
						(
							...
							'trans_comment'=>$sale_remarks,
							'trans_inventory_safe'=>0,
							'trans_inventory_floor'=>-$sale_item_kit_row['quantity_purchased'] * $item_kit_item->floor,
							'trans_current_safe' => $cur_item_location_info->safe ,
							'trans_current_floor' => $cur_item_location_info->floor - ($sale_item_kit_row['quantity_purchased'] * $item_kit_item->quantity),
							'trans_type' => 'Sale'
						);
	...
	{		
		...
		lang('common_helox') => 'common_helox',
	...
	function get_payment_options($cart)
	{
		...
		if ($this->Location->get_info_for_key('enable_credit_card_processing'))
		{
			$payment_options=array(
				...
				lang('common_helox') => lang('common_helox'),

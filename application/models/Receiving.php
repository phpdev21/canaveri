...
class Receiving extends CI_Model
{
	...
	function save ($cart)
	{
		...
		foreach($items as $line=>$item)
		{
			...
			if (!$cur_item_info->is_service)
			{
				...
				{
					$this->Item_location->save_quantity($cur_item_location_info->safe + $inventory_to_add,$cur_item_location_info->floor, $item->item_id,$cart->location_id ? $cart->location_id : ($cart->transfer_from_location_id ? $cart->transfer_from_location_id : $this->Employee->get_logged_in_employee_current_location_id()));
					...
					(
						...
						'trans_inventory_safe'=>$inventory_to_add,
						'trans_inventory_floor'=>0,							
						'location_id'=>$cart->location_id ? $cart->location_id : ($cart->transfer_from_location_id ? $cart->transfer_from_location_id : $this->Employee->get_logged_in_employee_current_location_id()),
						'trans_current_safe' => ($cur_item_location_info->safe ?  $cur_item_location_info->safe  : 0) + $inventory_to_add,
						'trans_current_floor' => ($cur_item_location_info->floor ?  $cur_item_location_info->floor  : 0),
			...
			if($suspended  == 0 && $mode=='transfer' && $location_id && $cur_item_location_info->safe !== NULL && !$cur_item_info->is_service)
			{					
				$item_weight = 0;
				$item_loc_qty_to_safe = $this->Item_location->get_location_quantity($item->item_id,$location_id) + ($item->quantity * -1);
				$item_loc_qty_to_floor = $this->Item_location->get_location_floor($item->item_id,$location_id);
				$this->Item_location->save_quantity($item_loc_qty_to_safe,$item_loc_qty_to_floor,$item->item_id,$location_id);
				...
				$inv_data['trans_inventory_safe']=($item->quantity*($item->quantity_unit_quantity !== NULL ? $item->quantity_unit_quantity : 1)) * -1;
				$inv_data['trans_inventory_floor']=0;
				$inv_data['location_id']=$location_id;
				$inv_data['trans_current_safe'] = $item_loc_qty_to_safe;
				$inv_data['trans_current_floor'] = $item_loc_qty_to_floor;
	  ...
	  if($recv_info['store_account_payment'])
		...
		if ($update_quantity)
		{
			...
			foreach($this->db->get()->result_array() as $receiving_item_row)
			{
				...
				{
					$this->Item_location->save_quantity($cur_item_location_info->safe - $inventory_to_remove,$cur_item_location_info->floor,$receiving_item_row['item_id'],$receiving_location_id);
					...
					(
						...
						'trans_user'=>$employee_id,
						'trans_comment'=>$recv_remarks,
						'trans_inventory_safe'=>$inventory_to_remove*-1,
						'trans_inventory_floor'=>0,
						'location_id'=>$receiving_location_id,
						'trans_current_safe' => $cur_item_location_info->safe - $inventory_to_remove,
				...
				{				
					...
					$this->Item_location->save_quantity($cur_item_location_transfer_info->safe + $receiving_item_row['quantity_purchased'],$cur_item_location_transfer_info->floor,$receiving_item_row['item_id'], $receiving_item_row['transfer_to_location_id']);
					...
					$inv_data = array
						...
						'trans_inventory_safe'=>$receiving_item_row['quantity_purchased'] * 1,
						'trans_inventory_floor'=>0,
						'location_id'=>$receiving_item_row['transfer_to_location_id'],
						'trans_current_safe' => $cur_item_location_transfer_info->safe + $receiving_item_row['quantity_purchased'],
						'trans_current_floor' => $cur_item_location_transfer_info->floor,
						'trans_type' => 'Inventory'
						);
					$this->Inventory->insert($inv_data);						
	...
	function undelete($receiving_id)
	{
		...
		foreach($this->db->get()->result_array() as $receiving_item_row)
		{
			...
			if ($inventory_to_add !=0)
			{
				$this->Item_location->save_quantity($cur_item_location_info->safe + $inventory_to_add,$cur_item_location_info->floor,$receiving_item_row['item_id'],$receiving_location_id);
				...
				(
					...
					'trans_inventory_floor'=>0,
					'location_id'=>$receiving_location_id,
					'trans_current_safe' => $cur_item_location_info->safe + $inventory_to_add,
					'trans_current_floor' => $cur_item_location_info->floor,
					'trans_type' => 'Inventory'
				);
				$this->Inventory->insert($inv_data);
	...
	function get_payment_options($cart)
	{
		$payment_options=array(
			...
			lang('common_term') => lang('common_term')
		);

...
class Category extends CI_Model
{
	...
	*/
	...
	function get_all_categories_and_sub_categories_as_tree()
	{
		...
		foreach ($categories as $k => $v) 
		{
			...
			$node->name = $v['name'];
			$node->enable_mmj_weight = $v['enable_mmj_weight'];
	...
	function get_all($parent_id = NULL, $show_hidden = FALSE, $limit=10000, $offset=0,$col='name',$order='asc',$can_use_cache = TRUE)
	{
		...
		if (!$can_use_cache || !$cache)
		{
		  ...
		  $this->db->order_by($col, $order);
			...
			{
				$cache[$result['parent_id'] ? $result['parent_id'] : 0][] = array('category_info_popup' => $result['category_info_popup'],'name' => $result['name'],'enable_mmj_weight' => $result['enable_mmj_weight'], 'color' => $result['color'], 'image_id' => $result['image_id'],'hide_from_grid' => $result['hide_from_grid'],'exclude_from_e_commerce' => $result['exclude_from_e_commerce'], 'parent_id' => $result['parent_id'], 'id' => $result['id']);
			}
		}
		...
		{	
			...
			{
				$return[$row['id']] = array('category_info_popup' => $row['category_info_popup'],'name' => $row['name'],'enable_mmj_weight' => $row['enable_mmj_weight'], 'color' => $row['color'], 'image_id' => $row['image_id'], 'hide_from_grid' => $row['hide_from_grid'],'exclude_from_e_commerce' => $row['exclude_from_e_commerce'], 'parent_id' => $row['parent_id'], 'depth' => NULL);
			}
		}
	...
	*/
	...
	function sold_type_and_mmj_info($category_id)
	{	
		$return_array = array();
		$this->db->from('categories');
		$this->db->where('id',$category_id);		
		$query = $this->db->get();
		$row = $query->row();
		$return_array['enable_mmj_weight'] = $row->enable_mmj_weight;
		return $return_array;		
	}
	...
	*/
	...
	function save($category_name = "", $hide_from_grid = NULL, $parent_id = NULL, $category_id = FALSE, $category_color = FALSE, $category_image_id = NULL,$system_category = 0,$exclude_from_e_commerce = 0,$category_info_popup = NULL,$enable_mmj_weight = 0)
	{
		if ($category_id == FALSE)
		{
			if ($category_name)
			{
				if($this->db->insert('categories',
					array('category_info_popup' => $category_info_popup,'name' => $category_name, 
					'exclude_from_e_commerce' => $exclude_from_e_commerce ? 1 : 0,'hide_from_grid' => $hide_from_grid ? 1 : 0, 
					'parent_id' => $parent_id, 'color' => $category_color ? $category_color : '',
					'image_id' => $category_image_id, 'last_modified' => date('Y-m-d H:i:s'),
					'system_category' => $system_category,'enable_mmj_weight' => $enable_mmj_weight ? 1 : 0
				)))
				{
		...
		else
		{
			...
			$update_data['enable_mmj_weight'] = $enable_mmj_weight ? 1 : 0;

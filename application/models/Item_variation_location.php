...
{	
	function get_variations_with_quantity($item_id, $location=false)
	{
		...
		foreach($this->db->get()->result_array() as $result)
		{
			...
			foreach($variations as $variation)
			{
				...
				$return[$item_variation_id]['item_weight'] = $variation['item_weight'];
				$return[$item_variation_id]['safe'] = $variation['safe'];
				$return[$item_variation_id]['floor'] = $variation['floor'];
	...
	function save_quantity($item_weight = null,$safe,$floor, $item_variation_id, $location_id=false)
	{
		if(!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id();
		...
		// Added checks to ensure the safe and floor values have legit numbers, as NOT NULL is a db constraint
		if ($safe == NULL) {
			$safe = 0;
		}
		if ($floor == NULL) {
		...
		$sql = 'INSERT INTO '.$this->db->dbprefix('location_item_variations'). ' (item_weight,safe,floor,item_variation_id, location_id)'
		    . ' VALUES (?, ?, ?,?,?)'
		    . ' ON DUPLICATE KEY UPDATE safe = ?,floor = ?'; 		
		return $this->db->query($sql, array($item_weight,$safe,$floor,$item_variation_id, $location_id,$safe,$floor));	
	}
	...
	*/
	...
	function get_location_floor($item_variation_id,$location=false)
	{
		if(!$location)
		{
			$location= $this->Employee->get_logged_in_employee_current_location_id();
	...
	function get_location_floor_quantity($item_variation_id,$location=false)
	{
		if(!$location)
		{
			$location= $this->Employee->get_logged_in_employee_current_location_id();

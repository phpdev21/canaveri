...
class Item extends CI_Model
{
	...
	*/
	...
	function get_displayable_columns()
	{
		$return  = array(
			...
			'product_id' => 										array('sort_column' => 'product_id','label' => lang('common_package_number')),
			...
			'safe' => 											array('sort_column' => 'safe','label' => lang('common_safe')),
			'floor' => 											array('sort_column' => 'floor','label' => lang('common_floor')),
	...
	*/
	function get_all($deleted=0,$limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		if ($attribute_count > 0 && !$this->config->item('speed_up_search_queries'))
		{
			...
			FROM (
				...
				`$phppos_location_items`.`cost_price` as `location_cost_price`, 
				`$phppos_location_items`.`unit_price` as `location_unit_price`, 
				`$phppos_location_items`.`safe` as `safe`, 
				`$phppos_location_items`.`floor` as `floor`, 
				`$phppos_tax_classes`.`name` as `tax_group`,
				IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0,
		...
		else
		{
			...
			if (!$this->config->item('speed_up_search_queries'))
			{
				...
				location_items.safe as safe,location_items.floor as floor,
	...
	function get_category_id($item_id)
	{
		$this->db->from('items');
		$this->db->where('item_id',$item_id);
		$query = $this->db->get();
	...
	*/
	...
	function get_item_search_suggestions($search,$deleted=0,$price_field = 'unit_price',$limit=25,$hide_inactive = false,$supplier_id=-1)
	{
		if (!trim($search))
		{
			return array();
		...
		for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++)
		{
			if ($this->get_custom_field($k))  
			{
				...
				if ($this->get_custom_field($k,'type') != 'date')
				{
					$this->db->select('location_items.safe as safe, location_items.floor as floor, custom_field_'.$k.'_value as custom_field, item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category,  supplier_brands.brand as brand_name', false);						
				}
				else
				{
					$this->db->select('location_items.safe as safe, location_items.floor as floor, FROM_UNIXTIME(custom_field_'.$k.'_value, "'.get_mysql_date_format().'") as custom_field, item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category,  supplier_brands.brand as brand_name', false);
				}
				...
				$this->db->join('supplier_brands', 'supplier_brands.supplier_brand_id = items.brand_id', 'left');
	 			$this->db->where('items.deleted',$deleted);
				...
				foreach($temp_suggestions as $key => $value)
				{
					$suggestions[]=array('value'=> $key, 'label' => $value['label'], 'image' => $value['image'], 'category' => $value['category'],'quantity' => to_quantity($value['quantity']),'safe' => $value['safe'],'floor' => $value['floor'], 'item_number' => $value['item_number'], 'variation_id' => $value['variation_id'], 'brand_name' => $value['brand_name']);
				}
			}			
		}
		...
		{	
			...
			$floor_quantity_field ="IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0,SUM(IF($phppos_item_variations.deleted=0, $phppos_location_item_variations.floor, 0)),$phppos_location_items.floor)";
			$safe_quantity_field ="IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0,SUM(IF($phppos_item_variations.deleted=0, $phppos_location_item_variations.safe, 0)),$phppos_location_items.safe)";
		}
		else
		{
			$quantity_field = 'NULL';
			$floor_quantity_field = 'NULL';
			$safe_quantity_field = 'NULL';
		}
		
		if($this->config->item('supports_full_text') && $this->config->item('enhanced_search_method'))
		{
			//-------------------------------BY NAME------------------------\\
			$this->db->select("$quantity_field as quantity,$floor_quantity_field as floor,$safe_quantity_field as safe,item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category, supplier_brands.brand as brand_name, MATCH (".$this->db->dbprefix('items').".name) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".name = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			$this->db->join('item_variations', 'items.item_id = item_variations.item_id','left');
			...
			foreach($temp_suggestions as $key => $value)
			{
				$suggestions[]=array('value'=> $key, 'label' => $value['label'], 'image' => $value['image'], 'category' => $value['category'],'quantity' => to_quantity($value['quantity']),'floor' => $value['floor'],'safe' => $value['safe'], 'item_number' => $value['item_number'], 'variation_id' => $value['variation_id'], 'brand_name' => $value['brand_name']);
			}
			//-------------------------------BY NAME------------------------\\
			
			//-------------------------------BY BRAND NAME------------------------\\
			  $this->db->select("$quantity_field as quantity,$floor_quantity_field as floor,$safe_quantity_field as safe,item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category, supplier_brands.brand as brand_name, MATCH (".$this->db->dbprefix('supplier_brands').".brand) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('supplier_brands').".brand = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			  $this->db->from('supplier_brands');
			  $this->db->join('items', 'items.brand_id = supplier_brands.supplier_brand_id','left');
			  $this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			  $this->db->join('item_variations', 'items.item_id = item_variations.item_id','left');
			  $this->db->join('location_item_variations', "`$phppos_location_item_variations`.`item_variation_id` = `$phppos_item_variations`.`id` and `$phppos_location_item_variations`.`location_id` = $current_location",'left');
			  $this->db->join('item_images', '('.$items_table.'.item_id = '.$item_images_table.'.item_id and '.$item_variations_table.'.id='.$item_images_table.'.item_variation_id) or ('.$items_table.'.item_id = '.$item_images_table.'.item_id and '.$item_images_table.'.item_variation_id IS NULL)', 'left');
			  ...
			  $this->db->group_start();
			  $this->db->where("MATCH (".$this->db->dbprefix('supplier_brands').".brand) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE)", NULL, FALSE);			
			  $this->db->group_end();
			  $this->db->limit($limit);
			  $this->db->order_by('rel DESC, exact_score DESC');
			  $this->db->group_by('items.item_id, item_variations.id');
			  $by_brand_name = $this->db->get();
				  
			  $temp_suggestions = array();
		  
			  foreach($by_brand_name->result() as $row)
			  {	
				  $data = array(
					  'image' => $row->image_id ?  app_file_url($row->image_id) : base_url()."assets/img/item.png" ,
					  'category' => $row->category,
            ...
            //-------------------------------BY BRAND NAME------------------------\\
			$this->db->select("$quantity_field as quantity,$floor_quantity_field as floor,$safe_quantity_field as safe,item_variations.item_number as item_variation_number,item_variations.deleted as variation_deleted,item_images.image_id,items.*,item_variations.id as item_variation_id, categories.name as category, supplier_brands.brand as brand_name, MATCH (".$this->db->dbprefix('items').".item_number) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".item_number = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			...
			$this->db->select("$quantity_field as quantity,$floor_quantity_field as floor,$safe_quantity_field as safe,item_variations.deleted as variation_deleted,item_images.image_id, items.*, item_variations.id as item_variation_id, categories.name as category,supplier_brands.brand as brand_name, MATCH (product_id) AGAINST (".$this->db->escape(escape_full_text_boolean_search($search).'*')." IN BOOLEAN MODE) as rel, CASE WHEN ".$this->db->dbprefix('items').".product_id = ".$this->db->escape($search)." THEN 1 ELSE 0 END AS exact_score", false);
			$this->db->from('items');
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->join('location_items', 'location_items.item_id = items.item_id and location_id='.$current_location, 'left');
			...
			$this->db->select("$quantity_field as quantity,$floor_quantity_field as floor,$safe_quantity_field as safe,item_images.image_id, item_variations.id as item_variation_id, additional_item_numbers.*, items.unit_price, items.cost_price, categories.name as category, supplier_brands.brand as brand_name", false);
			$this->db->from('additional_item_numbers');
	...
	*/
	...
	function search($search, $deleted=0,$category_id = false, $limit=20,$offset=0,$column='name',$orderby='asc', $fields = 'all')
	{
		...
		{	
			$quantity_field ="IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0,SUM(IF($phppos_item_variations.deleted=0, $phppos_location_item_variations.quantity, 0)),$phppos_location_items.quantity)";
			$safe_field ="IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0,SUM(IF($phppos_item_variations.deleted=0, $phppos_location_item_variations.safe, 0)),$phppos_location_items.safe)";
			$floor_field ="IF(SUM(if($phppos_item_variations.deleted=0,1, 0))  > 0,SUM(IF($phppos_item_variations.deleted=0, $phppos_location_item_variations.floor, 0)),$phppos_location_items.floor)";
		}
		else
		{
			$quantity_field = "$phppos_location_items.quantity";
			$safe_field = "$phppos_location_items.safe";
			$floor_field = "$phppos_location_items.floor";
		}
		...
		if ($attribute_count > 0 && !$this->config->item('speed_up_search_queries'))
		{
			...
			{			
				if ($search)
				{
					...
					if ($fields == $this->db->dbprefix('items').'.item_id' || $fields == $this->db->dbprefix('items').'.reorder_level' 
						|| $fields == $this->db->dbprefix('location_items').'.safe' || $fields == $this->db->dbprefix('location_items').'.floor'
						|| $fields == $this->db->dbprefix('items').'.cost_price' || $fields == $this->db->dbprefix('items').'.unit_price' || $fields == $this->db->dbprefix('items').'.promo_price' || $fields == $this->db->dbprefix('tags').'.name' || $fields == $this->db->dbprefix('suppliers').'.company_name' || $fields == $this->db->dbprefix('manufacturers').'.name')
					{
						...
						if ($fields == $this->db->dbprefix('location_items').'.safe')
						{
							$having_main_query = "HAVING safe = ".$this->db->escape($search);
							$search_main_query = "WHERE $phppos_items.deleted=$deleted and system_item = 0";
						}
						elseif ($fields == $this->db->dbprefix('location_items').'.floor')
						{
							$having_main_query = "HAVING floor = ".$this->db->escape($search);
							$search_main_query = "WHERE $phppos_items.deleted=$deleted and system_item = 0";
						}
			...
			FROM (
				...
				`$phppos_location_items`.`cost_price` as `location_cost_price`, `$phppos_location_items`.`unit_price` as `location_unit_price`, `$phppos_tax_classes`.`name` as `tax_group`,
			 	$safe_field as safe, $floor_field as floor
	        FROM `$phppos_items`
					$manufacturers_join
					$additional_item_numbers_join
					LEFT JOIN `$phppos_location_items` 
		...
		else
		{
			...
			if (!$this->config->item('speed_up_search_queries'))
			{
				...
				location_items.safe as safe, location_items.floor as floor,0 as variation_count, 0 as has_variations, 
				location_items.reorder_level as location_reorder_level,
				location_items.cost_price as location_cost_price,
				location_items.unit_price as location_unit_price,
				tax_classes.name as tax_group
	...
	function search_count_all($search, $deleted = 0,$category_id = FALSE, $limit=10000, $fields = 'all')
	{
		...
		location_items.quantity as quantity, location_items.safe as safe, location_items.floor as floor,
		location_items.reorder_level as location_reorder_level,
		location_items.cost_price as location_cost_price,
		location_items.unit_price as location_unit_price');
		$this->db->from('items');
	...
	{
		return array('item_id','item_number','name','category_id','cost_price','unit_price');
	...
	*/
	...
	function save_damaged_qty($damaged_date,$damaged_safe,$damaged_floor,$item_id,$item_variation_id = NULL,$location_id = FALSE, $sale_id = NULL)
	{
		if (!$location_id)
		{
			$location_id= $this->Employee->get_logged_in_employee_current_location_id();
		...
		$damaged_data = array(
			...
			'damaged_safe' => $damaged_safe,
			'damaged_floor' => $damaged_floor,
			'item_id' => $item_id,
			'item_variation_id' => $item_variation_id,
			'location_id' => $location_id,
			'sale_id' => $sale_id			
	...
	function get_items_for_cron()
	{
		$this->db->select('item_id,category_id,mmj_weight');
		$this->db->from('items');
		$this->db->where('items.deleted',0);
		$this->db->where('items.system_item',0);
	...
	function get_transactions_for_today($item_id){
		$current_date = date("Y-m-d");
		$this->db->select('trans_items,trans_current_safe,trans_current_floor,trans_date');
		$this->db->from('inventory');
		$this->db->where('DATE(trans_date)',$current_date);
		$this->db->where('trans_items',$item_id);
		return $this->db->get()->result_array();
	}
	
	function get_cron_previous_entry($item_id){
		$last = $this->db->order_by('inventory_cron_id',"desc")
	...
	function get_purchase_order_for_today($item_id){
		$current_date = date("Y-m-d");
		$this->db->select('quantity_received,receivings.receiving_time');
		$this->db->from('receivings_items');
		$this->db->join('receivings', 'receivings.receiving_id = receivings_items.receiving_id','left');
		$this->db->where('DATE(receiving_time)',$current_date);
		$this->db->where('receivings.is_po',0);
		$this->db->where('receivings_items.item_id',$item_id);
		return $this->db->get()->result_array();
	}

	function get_sales_order_for_today($item_id){

		$current_date = date("Y-m-d");
		$this->db->select('quantity_purchased,sales.sale_time');
		$this->db->from('sales_items');
		$this->db->join('sales', 'sales.sale_id = sales_items.sale_id','left');
		$this->db->where('DATE(sale_time)',$current_date);
		$this->db->where('sales_items.item_id',$item_id);
		return $this->db->get()->result_array();
	}
	
	// function if_variation_exists($item_id){
	// 	$this->db->from('item_variations');
	// 	$this->db->where('item_variations.item_id',$item_id);	
	// 	$data = count($this->db->get()->result_array());	 

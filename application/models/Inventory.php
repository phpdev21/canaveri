...
{	
	...
	{
		if(is_numeric($inventory_data['trans_inventory_safe']) || is_numeric($inventory_data['trans_inventory_floor']))
		{
	...
	function update_inventory_from_count($count_id)
	{
		...
		foreach($this->get_items_counted($count_id, NULL,NULL) as $count_item)
		{
			...
			if ($current_inventory_value != $counted_inventory_value)
			{
				...
				$cur_safe = $this->Item_location->get_location_quantity($count_item['item_id'],$count_item['location_id']);
				$cur_floor = $this->Item_location->get_location_floor($count_item['item_id'],$count_item['location_id']);
				$this->Item_location->save_quantity($cur_safe + ($counted_inventory_value - $current_inventory_value),$cur_floor, $count_item['item_id'],$count_item['location_id']);

...
<div class="row register" id="receiving-parent" data-supplier-id="<?php echo $supplier_id; ?>">
								...
								<td colspan="5">
									<dl class="register-item-extra-details dl-horizontal">
										...
										if (count($serial_numbers) > 0){
											?>
											<dd  class=""><a href="#" id="serialnumber_<?php echo $line;?>"data-name="serialnumber" data-type="select" data-pk="1" data-url="<?php echo site_url('receivings/edit_item/'.$line); ?>" data-title="<?php echo H(lang('common_package_number')); ?>"><?php echo character_limiter(H($item->serialnumber), 50); ?></a></dd>
											<?php }
										else
										{
										?>
									  <dd  class="">
											<a href="#" id="serialnumber_<?php echo $line;?>" class="xeditable" data-type="text" data-pk="1" data-name="serialnumber" data-value="<?php echo H($item->serialnumber); ?>" data-url="<?php echo site_url('receivings/edit_item/'.$line); ?>" data-title="<?php echo H(lang('common_package_number')); ?>"><?php echo character_limiter(H($item->serialnumber), 50); ?></a>
										</dd>
										<?php
										}
										?><?php
													...
													if(newValue == -1)
													{
														...
														bootbox.prompt({
															...
															callback: function(serial_number) {
													  		if (serial_number)
													  		{
																	$.post(<?php echo json_encode(site_url('receivings/edit_item/'.$line)); ?>, {name: 'serialnumber',value: serial_number}, function(response)
																	{
													 					$("#register_container").html(response);
																	});
													  		}
										...
										{ 
											...
											$cur_safe = $item_location_info->safe;
											$cur_floor = $item_location_info->floor;
											
											
											?>
											<dt><?php echo lang('common_safe_stock'); ?></dt>
											<dd><?php echo to_quantity($cur_safe); ?></dd>

											<dt><?php echo lang('common_floor_stock'); ?></dt>
											<dd><?php echo to_quantity($cur_floor); ?></dd>
	...
	<!-- Receive  Top Buttons  -->
		...
		<?php }  else {  ?>
			<?php if(isset($supplier)) {  ?>
				<!-- Customer Badge when customer is added via supplier -->
   ...
   $("#change_cart_date").on("dp.change", function(e) {
	...
	$('#select_supplier_form,#select_location_form').ajaxForm({target: "#register_container", beforeSubmit: receivingsBeforeSubmit});
			...
			$( "#item" ).autocomplete({
                ...
                {
		 		    $.getJSON('<?php echo site_url("receivings/item_search");?>', {
                        term: decodeHtml(request.term),
             ...
             .data("item.autocomplete", item)
	           .append('<a class="suggest-item" data-value="' + item.value + '" data-attributes="' + item.attributes + '"><div class="item-image">' +
						...
						'<div class="details">' +
							...
							(typeof item.safe !== 'undefined' && item.safe!==null ? '<span class="attributes">' + <?php echo json_encode(lang('common_safe')); ?> +' '+ (<?php echo json_encode(lang('common_quantity')); ?> ) + ': <span class="value">'+ parseFloat(item.safe) + '</span></span>' : '' )+
							(typeof item.floor !== 'undefined' && item.floor!==null ? '<span class="attributes">' + <?php echo json_encode(lang('common_floor')); ?> + ' ' + (<?php echo json_encode(lang('common_quantity')); ?> ) + ': <span class="value">'+ parseFloat(item.floor) + '</span></span>' : '' )+
...
function getSupplierId() {
    var recParent = $('#receiving-parent');
    if (recParent !== undefined && recParent !== null && recParent.length > 0) {
        var supplierId = recParent[0].dataset.supplierId;
...
function checkPaymentTypes()
{
  ...
  case <?php echo json_encode(lang('common_cash')); ?>:
      ...
      break;
  case <?php echo json_encode(lang('common_term')); ?>:
        $("#amount_tendered").val(<?php echo json_encode(to_currency_no_money($amount_due)); ?>);
        $("#amount_tendered").attr('placeholder',<?php echo json_encode(lang('common_enter').' '.lang('common_term').' '.lang('common_amount')); ?>);
        break;

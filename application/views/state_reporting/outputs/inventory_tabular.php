					...
					<table class="table table-bordered table-striped table-reports tablesorter" id="sortable_table">
						...
						<tbody>
							...
							<tr>
								...
								<?php if($cell['category_id'] && $cell['data'] != ''){?>
									<a target="_blank" href="<?php echo site_url('state_reporting/generate/detailed_inventory_report/?category_id='.$cell['category_id'].'&date='.$selected_date.'&category='.$cell['category']);?>" >
										<?php echo $cell['data']; ?>
									</a>
								<?php }else{ 
									echo ($cell['data'] == '') ? '-' : $cell['data']  ; 
								}?>
								</td>
								<?php $i++; } ?>
							</tr>							

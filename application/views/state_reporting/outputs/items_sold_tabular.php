...
if($export_excel == 1)
{
	...
	foreach ($headers as $header) 
	{
		...
		if($header['col_type'] !== '')
		{
			$column_type = ($header['col_type'] === 'weight') ? 'Weight (lbs)' : 'Count-Based sales'; 
	...
	$actual_link = "https://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
		if (strpos($actual_link,'items_sold_report') !== false) {
			$sheet_type = 'csv';
		} else {
			$sheet_type = $this->config->item('spreadsheet_format');
		}
	array_to_spreadsheet($rows, strip_tags($title) . '.'.($sheet_type == 'XLSX' ? 'xlsx' : 'csv'), true);
	exit;
}
?>
					...
					<table class="table table-bordered table-striped table-reports tablesorter" id="sortable_table">
						<thead>
							<tr>
								...
								<th align="<?php echo $header['align'];?>" class="colsho <?php echo $header['column_id']; ?>" style="<?php if($header['view'] == 0) { ?>display:none;<?php } ?>">
									...
									<?php
										if($header['col_type'] !== ''){ ?>
											<span>	<?php echo ($header['col_type'] === 'weight') ? 'Weight (lbs)' : 'Count-Based sales'; ?></span>
									<?php
									}
									?>
								</th>
						...
						<tbody>
							...
							<tr>
								...
								<td align="<?php echo $cell['align'];?>" class="colsho <?php echo $i; ?>">
									<?php 
									if($cell['data'] != '')
									{ 
										if($cell['category_id'] && $cell['data'] != 0){
										?>
										<a target="_blank" href="<?php echo site_url('state_reporting/generate/detailed_item_sold_report/?category_id='.$cell['category_id'].'&start_date='.$start_date.'&end_date='.$end_date.'&category='.$cell['category']);?>" >
										<?php echo $cell['data']; ?>
										</a>
										<?php
										}else{
...
{
	var divToPrint = document.getElementById('sortable_table');

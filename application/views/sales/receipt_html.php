...
?>
    ...
    <table width="100%">
        <tr>
            <td colspan="2" valign="top" align="left" width="160">
                ...
                <p><?php echo H($this->Location->get_info_for_key('name', isset($override_location_id) ? $override_location_id : FALSE)); ?>
                </p>
                <?php } ?>

                <p><?php echo nl2br(H($this->Location->get_info_for_key('address', isset($override_location_id) ? $override_location_id : FALSE))); ?>
                </p>

                <p><?php echo H($this->Location->get_info_for_key('phone', isset($override_location_id) ? $override_location_id : FALSE)); ?>
                </p>
                <?php if ($website) { ?>
            ...
            <td colspan="2" valign="top" align="center" width="160">
                ...
                <p><span class="text-danger"
                        style="color: #df6c6e;"><strong><?php echo lang('sales_deleted_voided'); ?></strong></span></p>
                <?php } ?>
        ...
        <tr>
            <td colspan="3" valign="top" align="left">
                ...
                <p><?php echo lang('common_name') . ": " . H($delivery_person_info['first_name'] . ' ' . $delivery_person_info['last_name']); ?>
                </p>
    ...
    ?>
        ...
        <tr>
            <td colspan="4">
                <p class="invoice-head item-name"><?php echo lang('common_item_name'); ?></p>
            </td>
        </tr>
        <tr>
            <td align="right">
                <p class="invoice-head text-right item-price">
                    <?php echo lang('common_price') . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . lang('common_tax') : ''); ?>
                </p>
            </td>
            <td align="right">
                <p class="invoice-head text-right item-qty"><?php echo lang('common_quantity'); ?></p>
            </td>
            <td align="right">
            ...
            <td align="right">
                <p class="invoice-head pull-right item-total gift_receipt_element">
                    <?php echo lang('common_total') . ($this->config->item('show_tax_per_item_on_receipt') ? '/' . lang('common_tax') : ''); ?>
                </p>
            </td>
        </tr>
...
foreach (array_reverse($cart_items, true) as $line => $item) {
    ...
    if ($item->quantity > 0 && $item->name != lang('common_store_account_payment') && $item->name != lang('common_discount') && $item->name != lang('common_refund') && $item->name != lang('common_fee')) {
        ...
        array_push($total_items_weight,($item->quantity * explode(" ",$item->variation_name)[0]));
...
?>
        ...
        <tr>
            <td colspan="3" align="right">
                <div class="invoice-footer-heading"><?php echo lang('common_exchange_to') . ' ' . H($exchange_name); ?>
                </div>
            </td>
            <td align="right">
    ...
    ?>
        ...
        <tr>
            ...
            <td align="right">
                <div class="invoice-footer-value invoice-total"><?php echo to_quantity($number_of_items_returned); ?>
                </div>
            </td>
            <?php } ?>
        </tr>
        <tr>
            <?php if ($total_items_weight) { ?>
            <td colspan="3" align="right">
                <div class="invoice-footer-heading"><?php echo lang('common_weight_sold'); ?></div>
            </td>
            <td align="right">
                <div class="invoice-footer-value invoice-total"><?php echo array_sum($total_items_weight).' Gram'; ?>
                </div>
            </td>
            <?php } ?>
        </tr>
    ...
    ?>
        <tr>
            <td colspan="3" align="right">
                ...
                <?php } else { ?>
                <div class="invoice-footer-value">
                    &nbsp;<?php $splitpayment = explode(':', $payment->payment_type); echo H($splitpayment[0]); ?></div>
                <?php } ?>
            </td>
        ...
        <tr>
            ...
            <td colspan="4">
                ...
                <div class="invoice-footer-value invoice-footer-value-cc">
                    <?php echo lang('sales_entry_method') . ': ' . H($payment->entry_method); ?></div>
                <?php } ?>

                <?php if ($payment->tran_type) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc">
                    <?php echo lang('sales_transaction_type') . ': ' . ($is_sale_integrated_ebt_sale ? 'EBT ' : '') . H($payment->tran_type); ?>
                </div>
                <?php } ?>

                <?php if ($payment->application_label) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc">
                    <?php echo lang('sales_application_label') . ': ' . H($payment->application_label); ?></div>
                <?php } ?>

                <?php if ($payment->ref_no) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc">
                    <?php echo lang('sales_ref_no') . ': ' . H($payment->ref_no); ?></div>
                <?php } ?>
                <?php if ($payment->auth_code) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc">
                    <?php echo lang('sales_auth_code') . ': ' . H($payment->auth_code); ?></div>
                <?php } ?>

                <?php if ($payment->aid) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc"><?php echo 'AID: ' . H($payment->aid); ?>
                </div>
                <?php } ?>

                <?php if ($payment->tvr) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc"><?php echo 'TVR: ' . H($payment->tvr); ?>
                </div>
                <?php } ?>


                <?php if ($payment->tsi) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc"><?php echo 'TSI: ' . H($payment->tsi); ?>
                </div>
                <?php } ?>


                <?php if ($payment->arc) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc"><?php echo 'ARC: ' . H($payment->arc); ?>
                </div>
                <?php } ?>

                <?php if ($payment->cvm) { ?>
                <div class="invoice-footer-value invoice-footer-value-cc"><?php echo 'CVM: ' . H($payment->cvm); ?>
                </div>
                <?php } ?>
            </td>
            <?php } ?>
...
?>
        ...
        <tr>
            ...
            <td align="right">
                <div class="invoice-footer-value invoice-payment">
                    <?php echo to_currency($this->Giftcard->get_giftcard_value(end($giftcard_payment_row))); ?></div>
            </td>
        </tr>
        <?php } ?>
        <?php } ?>
...
?>
        ...
        <tr>
            ...
            <td align="right">
                <div class="invoice-footer-value invoice-total"><?php echo to_currency($customer_balance_for_sale); ?>
                </div>
            </td>
        </tr>
        <?php
...
?>
        ...
        <tr>
            ...
            <td align="right">
                <div class="invoice-footer-value invoice-total">
                    <?php echo $sales_until_discount <= 0 ? lang('sales_redeem_discount_for_next_sale') : to_quantity($sales_until_discount); ?>
                </div>
            </td>
        </tr>
        <?php
...
if ($this->config->item('show_total_discount_on_receipt')) { ?>
        <tr>
            ...
            <td align="right">
                <div class="invoice-footer-value invoice-total"><?php echo to_currency($cart->get_total_discount()); ?>
                </div>
            </td>
        </tr>
        <?php

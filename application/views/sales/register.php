							...
							<tr class="register-item-bottom">
								...
								<td colspan="5">
									<dl class="register-item-extra-details dl-horizontal">
										...
										<dd id="category_<?php echo $line;?>"><?php echo $this->Category->get_full_path($item->category_id)?></dd>
										...
										<dt  class=""><?php echo lang('common_package_number'); ?> </dt>
										<?php
										//$serial_numbers = $this->Item_serial_number->get_all($item->item_id);
										$this->db->select("product_id");
										$this->db->from('items');
										$this->db->where('item_id', $item->item_id);
										$serial_numbers =  $this->db->get()->result_array()[0]['product_id'];
										$serial_numbers = json_decode($serial_numbers);
										$source_data = array();
										if (count($serial_numbers) > 0)
										{
											?>
										  <dd  class=""><a href="#" id="serialnumber_<?php echo $line;?>"data-name="serialnumber" data-type="select" data-pk="1" data-url="<?php echo site_url('sales/edit_item/'.$line); ?>" data-title="<?php echo H(lang('common_package_number')); ?>"><?php echo character_limiter(H($item->serialnumber), 50); ?></a></dd>
										<?php
										}
										else
										{
										?>
									  <dd  class="">
											<a href="#" id="serialnumber_<?php echo $line;?>" class="xeditable" data-type="text" data-pk="1" data-name="serialnumber" data-value="<?php echo H($item->serialnumber); ?>" data-url="<?php echo site_url('sales/edit_item/'.$line); ?>" data-title="<?php echo H(lang('common_package_number')); ?>"><?php echo character_limiter(H($item->serialnumber), 50); ?></a>
										</dd>
										<?php
										}
										?>
										...
										if (count($serial_numbers) > 0)
										{
											$source_data[] = array('value' => '-1', 'text' => lang('sales_enter_package_number'));
											
											foreach($serial_numbers as $serial_number)
											{
												// $source_data[] = array('value' => $serial_number['serial_number'], 'text' => $serial_number['serial_number']);
												$source_data[] = array('value' => $serial_number, 'text' => $serial_number);
											}
											?>
											<script>												
										  $('#serialnumber_<?php echo $line;?>').editable({
									    	...
									    	success: function(response, newValue) {
													if(newValue == -1)
													{
														...
														bootbox.prompt({
															title: <?php echo json_encode(lang('sales_enter_package_number')); ?>,
															inputType: 'text',
															value: '',
										...
										{ 
											...
											$cur_quantity = $item_location_info->floor;
										...
										<dt><?php echo lang('common_label');?></dt>
										<dd>
										<?php 
											// echo $item->item_kit_id;

											$printSingleLink = "sales/labelSingle/";
											if(isset($item->item_kit_id)){
												$printSingleLink .= 'item_kit_id/'.$item->item_kit_id;
											}elseif(isset($item->item_id)){
												$printSingleLink .= 'item_id/'.$item->item_id;
											}

										 	echo anchor($printSingleLink, ''.lang('common_print') .' ' . lang('common_label').'</span>',
											array('id'=>'print_label', 'class'=>'','title'=> lang('common_print') .' ' . lang('common_label'))); ?>
										</dd>
										<?php } ?>
			...
			<div class="sale-buttons">
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu sales-dropdown" role="menu">						
						...
						<li>
							<?php echo anchor("sales/labels", '<span class="ion-android-print"> '.lang('common_print') .' ' . lang('common_label').'</span>',
								array('id'=>'print_label', 'class'=>'','title'=> lang('common_print') .' ' . lang('common_label'))); ?>
						</li>
...
<?php if(count($cart_items) > 0){ ?> 
		...
		<?php if ($customer_required_check) { ?>
			<div class="add-payment">
				...
				?>
					<a tabindex="-1" href="#" class="<?php if($value == "Helox"){?>tooltip_helox <?php }?>btn btn-pay select-payment <?php echo $active_payment; ?>" data-payment="<?php echo H($value); ?>" helox-prefix="<?php echo $helox_payment_prefix; ?>" order-id="<?php echo $last_sale_id; ?>">
					<?php if($value == "Helox"){?>
					<span class="cstm-tooltip">You should have to pay full amount with Helox</span>
					<?php } ?>
					<?php echo H($value); ?>
					</a>
				<?php } ?>
...
<div class="modal fade look-up-receipt" id="choose_var" role="dialog" aria-labelledby="lookUpReceipt" aria-hidden="true">
	...
	<div class="loader_screen">
		<div class="">
			...
			<p class="text-center loader-warning">Don't refresh the page</p>
			<p class="text-center loader-text">A payment link will sent to your mobile number</p>
			<p class="text-center helper-loader">This payment link automatically expired after 5-10 minutes.</p>
		</div>
	</div>
<div class="modal fade look-up-receipt" id="complete_sale" role="dialog" aria-labelledby="lookUpReceipt" aria-hidden="true" helox_status="">
    <div class="modal-dialog customer-recent-sales">
      	<div class="modal-content">
	        <div class="modal-header">
	          	<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span></button>
	          	<h4 class="modal-title" id="lookUpReceipt"><?php echo lang('sales_complete_currrent_with_helox_sale'); ?></h4>
	        </div>
	        <div class="modal-body clearfix">
				<div class="">
					<div class="row">
						<div class="col-md-6">
							<div class="m-b-20">
								...
								<input type="text" id="helox_order_id" value="" class="form-control" readonly>
							</div>
						</div>
						<div class="col-md-6">
							<div class="m-b-20">
								<label>Amount</label>
								<input type="text" id="helox_amount"value="" class="form-control" readonly>
							</div>
						</div>
						<div class="col-md-6">
							<div class="m-b-20">
								<label>Payment Type</label>
								<input type="text" value="Helox" class="form-control" readonly>
							</div>
						</div>
						<div class="col-md-6">
							<div class="m-b-20">
								<label>Mobile Number</label>
								<input type="number" id="helox_mobile_number"value="" class="form-control">
							</div>
						</div>
					</div>
					<div class="text-center">
						<a href="#" class="btn btn-success" id="complete_holex_payments">Confirm</a>
					</div>
				</div>
	        </div>
    	</div><!-- /.modal-content -->
	...
	$('.xeditable').on('hidden', function(e, editable) {
	 ...
	 $('.xeditable').on('hidden', function(e, editable) {
	             ...
	             .data("item.autocomplete", item)
		           .append('<a class="suggest-item"><div class="item-image">' +
							...
							'<div class="details">' +
								...
								(typeof item.floor !== 'undefined' && item.floor!==null ? '<span class="attributes">' + ( <?php echo json_encode(lang('common_quantity')); ?> ) + ': <span class="value">'+ parseFloat(item.floor) + '</span></span>' : '' )+
								(typeof item.brand_name !== 'undefined' && item.brand_name!==null ? '<span class="attributes">' + '<?php echo lang("common_brand"); ?>' + ': <span class="value">'+item.brand_name + '</span></span>' : '' )+
				...
				delay: 150,
		 		...
		 		select: function( event, ui ) 
		 		{
					if(ui.item.rep_exp){
						...
						var endDate= new Date(Date.parse(ui.item.rep_exp));
						if (TodayDate > endDate) {
							$("body").append('<div id="toast-container" class="toast-top-right" aria-live="polite" role="alert"><div class="toast hidden-print toast-error" style="display: block;"><div class="toast-title">Selected Customer Medical Card is expired or not available.!</div><div class="toast-message">warning</div></div></div>');
							$.post('<?php echo site_url("sales/select_customer");?>', {customer: '' }, function(response)
							{
								$("#register_container").html(response);
							});
							setTimeout(function () {
      ...
      $("#change_cart_date").on("dp.change", function(e) {
		...
		{
			e.preventDefault();
			$total_items =  parseInt($(".cart-number").text());
			var obj = {};
			for(var i = 0; i < $total_items; i++) 
			{
				if($("#variation_"+i).text()){
					var gram =  parseInt($("#variation_"+i).text().split(" ")[0]);
				}else{
					var gram =  1;
				}
				var quantity =  parseInt($("#quantity_"+i).text());
				var total_weight = gram*quantity;
				var category = $("#category_"+i).text().replaceAll(' ','');
				if(category in obj){
					obj[category] = obj[category]+ total_weight;
				}else{
					obj[category] = total_weight;	
				}
			}  
			var restirct_category = '<?php echo $section_purchase_settings; ?>';
			var rv = true;
			if(restirct_category !=""){
				var restirct_category_array = JSON.parse(restirct_category);
				jQuery.each(restirct_category_array, function(index, item) {
					if(obj[item[0]]){
			...
			if(!rv){
				...
				jQuery.each(restirct_category_array, function(index, item) {
					if(obj[item[0]]){
						if(obj[item[0]] >= item[1]){
							return message =  'Purchase not to exceed '+item[1]+ ' grams for category '+item[0]
						} 
					}
				});
				$('body').append('<div id="toast-container" class="toast-top-right" aria-live="polite" role="alert"><div class="toast hidden-print toast-error" style="display: block; width:400px;"><div class="toast-title">Error Items should follow given weight criteria:</div><div class="toast-message"><p>'+message+'</p></div></div></div>');
					setTimeout(function () {
                     $('#toast-container').remove();
                 }, 2500);
			}else{
		...
		$('#finish_sale_alternate_button').on('click',function(e){
			e.preventDefault()
			var active_payment_method = $('.select-payment.active').attr('data-payment');
			var helox_payment_status = $('#complete_sale').attr('helox_status');
			if(active_payment_method == 'Helox' && helox_payment_status == "")
			{
				var order_id = parseInt($('.select-payment.active').attr('order-id'))+1;
				var helox_pefix = $('.select-payment.active').attr('helox-prefix');
				var $total_amount = $("#amount_tendered").val();
				$("#helox_order_id").val(helox_pefix+"-"+order_id);
				$("#helox_amount").val($total_amount);
				$('#complete_sale').modal('show');
			}
			else
			{
				$total_items =  parseInt($(".cart-number").text());
...
function noPaymentSelected()
{
	...
	$(document).ready(function(){
		if($('.select-payment.active').attr('data-payment') == 'Helox'){
			$("#amount_tendered").attr('readonly','readonly');
			$("#subtotal").addClass("disabled_total");
			$("#amount_tendered").css({ 'background-color': '#D7DCE5'});
		}
		$(".select-payment").click(function(){
			if($(this).attr('data-payment') == 'Helox'){
				$("#amount_tendered").attr('readonly','readonly');
				$("#subtotal").addClass("disabled_total");
				$("#amount_tendered").css({ 'background-color': '#D7DCE5'});
			}else{
				$("#amount_tendered").attr("readonly", false); 
				$("#subtotal").removeClass("disabled_total");
				$("#amount_tendered").css({ 'background-color': '#FFF'});
			}
		});

		// Confirm a helox payment with the total amount 
		$("#complete_holex_payments").click(function(){
			$('.loader_screen').css('display' , 'flex');
			$('#complete_sale').modal('hide'); 
			event.preventDefault();
			var order_id = $('#helox_order_id').val();
			var price = $('#amount_tendered').val();
			var mobile_number = $('#helox_mobile_number').val();
			jQuery.ajax({
				type: "post",
				url: "<?php echo site_url('sales/helox_order_payments'); ?>",
				data: {
					"order_id": order_id,
					"price": price,
				...
				success: function(response){
						setInterval(function () {
							jQuery.ajax({
								type: "post",
								url: "<?php echo site_url('sales/helox_order_payment_status'); ?>",
								data: {
									"order_id": order_id,
								},
								cache: false,
								success: function(response){
									...
									if( status.status == 'COMPLETED'){
										$('#complete_sale').attr('helox_status','done');

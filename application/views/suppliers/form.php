	...
	<?php echo form_open_multipart('suppliers/save/'.$person_info->person_id,array('id'=>'supplier_form','class'=>'form-horizontal')); ?>
			...
			<div class="panel-body">
				<div class="form-group">
					...
					<div class="form-group">	
						<?php echo form_label(lang('suppliers_brand_name').':', 'brands',array('class'=>'required col-sm-3 col-md-3 col-lg-2 control-label')); ?>
						<div class="col-sm-9 col-md-9 col-lg-10 form-group" id="brans_list_form">
							<?php if($this->uri->segment(3) == -1){ ?>
							<div class="input_form_row">
								<div class="col-sm-9 col-md-9 col-lg-10" style="margin-top:-1px;">							
									<?php echo form_input(array(
										'class'=>'form-control form-inps',
										'name'=>'brands[]',
								...
								<div class="col-sm-3 col-md-3 col-lg-2">	
									<!-- <input type="button" value="Delete"  class="removeBrand" style="margin-top:1.5rem;"/>								 -->
								</div>
							</div>
						<?php
						}else{
						...
						?>
							<div class="input_form_row">
								<div class="col-sm-9 col-md-9 col-lg-10" style="margin-top:1rem;">
							    	<input type="text" disabled class="form-control form-inps" name="brands[]" placeholder = "Brand Name" value="<?php echo $brand['brand']; ?>" />
								</div>
								<div class="col-sm-3 col-md-3 col-lg-2">
								   <input type="button" value="Delete" id="<?php echo $brand['supplier_brand_id']?>"  class="removeBrand" style="margin-top:1.5rem;"/>
								</div>
							</div>
						<?php
						   } 
					...
					<div class="form-group" style="margin-top:-2rem">	
						...
						<div class="col-sm-9 col-md-9 col-lg-10 ">
							<input type="button" value="Add Brand" id="addBrand" />
						</div>
					</div>
...
$(document).ready(function()
{
	$("#addBrand").click(function() {    
        $("#brans_list_form").append('<div class="input_form_row"><div class="col-sm-9 col-md-9 col-lg-10" style="margin-top:1rem;"><input type="text" class="form-control form-inps" name="brands[]" placeholder="Brand Name" value=""/></div><div class="col-sm-3 col-md-3 col-lg-2"><input type="button" value="Delete" id="0" class="removeBrand" style="margin-top:1.5rem;"/></div></div>');
    }); 

	$(document).on("click", ".removeBrand", function(){
		var row_count = $('.input_form_row').length; 
		if(row_count == 1)
		{
			alert('cannot delete!!!');
		}
		else{
			var supplier_brand_id = $(this).attr("id");
			if(supplier_brand_id > 0)
			{
				$("#supplier_form").append('<input type="hidden" name="supplier_brands_to_delete[]" value="'+ supplier_brand_id +'" />');
			}
			$(this).closest("div.input_form_row").remove();
		}
	});	


	var default_brands = $("input[name='selected_brands[]']")
              .map(function(){return $(this).val();}).get();

	if(default_brands)
	{
		$("#brands").val(default_brands);
	}
	...
	$(document).on('click', "#add_brand", function () {
         $("#brandModalDialogTitle").html(<?php echo json_encode(lang('common_add_brand')); ?>);
        // //Clear form
         $("#brand_form").find('#brand_name').val("");
        // //show
        $("#brand-input-data").modal('show');
    });
	...
	$("#brand_form").submit(function (event) {
        ...
        $(this).ajaxSubmit({
            success: function (response, statusText, xhr, $form) {
                ...
                if (response.success) {
                     $("#brand-input-data").modal('hide');
					 $("#brands").append('<option value="'+response.brands.brand_id+'">'+response.brands.brand_name+'</option>');
					 var brandval = $("#brands").val() || [];
                     brandval.push(response.selected);
					 $("#brands").val(brandval);
		...
		rules: 
		{
			...
			messages: 
			{
				<?php if(!$person_info->person_id) { ?>
					...
					"brands[]": <?php echo json_encode(lang('brand_name_required')); ?>,
